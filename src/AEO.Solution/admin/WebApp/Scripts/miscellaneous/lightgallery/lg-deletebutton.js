(function (root, factory) {
  ...
  (function () {
    ...
    Delete.prototype.init = function () {
      ...
      var deleteIcon = '<button type="button" aria-label="Toggle deletePicture" class="lg-icon deletePicture"><i class="fal fa-trash-alt"></i></button>';
    ...
    Delete.prototype.delete = function () {
      ...
      this.core.$outer.find('.deletePicture').on('click', function () {
        ...
        const removed = elements.splice(that.core.index, 1);
        if (that.core.s.onDeletePicture instanceof Function) {
          that.core.s.onDeletePicture(removed)
        }
        if (!that.core.s.dynamic) {
