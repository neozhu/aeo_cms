...
/// </summary>
   public class ProductPrictureQuery:QueryObject<ProductPricture>
   {
		public ProductPrictureQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "LineNo" && !string.IsNullOrEmpty(rule.value) && rule.value.IsInt())
						{
							var val = Convert.ToInt32(rule.value);
							switch (rule.op) {
                            case "equal":
         ...
         public  ProductPrictureQuery ByProductIdWithfilter(int productid, IEnumerable<filterRule> filters)
         {
