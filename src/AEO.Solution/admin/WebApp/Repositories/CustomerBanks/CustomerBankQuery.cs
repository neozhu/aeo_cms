...
/// </summary>
   public class CustomerBankQuery:QueryObject<CustomerBank>
   {
		public CustomerBankQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "AccountName"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.AccountName.Contains(rule.value));
						}
						if (rule.field == "Bank"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.Bank.Contains(rule.value));
						}
						if (rule.field == "AccountNo"  && !string.IsNullOrEmpty(rule.value))
						{
						...
						{
							And(x => x.SWIFT.Contains(rule.value));
						}
         ...
         public  CustomerBankQuery ByCustomerIdWithfilter(int customerid, IEnumerable<filterRule> filters)
         {
