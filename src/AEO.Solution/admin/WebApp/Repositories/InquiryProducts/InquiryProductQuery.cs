...
{
/// <summary>
/// File: InquiryProductQuery.cs
/// Purpose: easyui datagrid filter query 
/// Created Date: 2020/8/19 10:56:18
/// Author: neo.zhu
/// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
/// Copyright (c) 2012-2018 All Rights Reserved
/// </summary>
   public class InquiryProductQuery:QueryObject<InquiryProduct>
   {
		public InquiryProductQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "SupplierProductNo"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.SupplierProductNo.Contains(rule.value));
						}
						...
						if (rule.field == "MinQty" && !string.IsNullOrEmpty(rule.value) && rule.value.IsDecimal())
						{
							var val = Convert.ToDecimal(rule.value);
							switch (rule.op) {
                            case "equal":
						...
						if (rule.field == "PriceDate" && !string.IsNullOrEmpty(rule.value) )
						{	
							if (rule.op == "between")
                            {
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.PriceDate) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.PriceDate) <= 0);
						    }
						}
						if (rule.field == "Feedback"  && !string.IsNullOrEmpty(rule.value))
						{
							And(x => x.Feedback.Contains(rule.value));
						}
						if (rule.field == "Recommended" && !string.IsNullOrEmpty(rule.value) && rule.value.IsBool())
						{	
							 var boolval=Convert.ToBoolean(rule.value);
							 And(x => x.Recommended == boolval);
						}
         ...
         public  InquiryProductQuery ByInquiryIdWithfilter(int inquiryid, IEnumerable<filterRule> filters)
         {
            ...
            if (filters != null)
           {
               ...
               {     
						...
						if (rule.field == "SupplierProductNo"  && !string.IsNullOrEmpty(rule.value))
						{
                           if (rule.op == "equal")
                           {
                             And(x => x.SupplierProductNo == rule.value);
                           } 
                           else
                           {
							And(x => x.SupplierProductNo.Contains(rule.value));
