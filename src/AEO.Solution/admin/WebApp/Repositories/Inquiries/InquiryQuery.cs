...
/// </summary>
   public class InquiryQuery:QueryObject<Inquiry>
   {
		public InquiryQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
               foreach (var rule in filters)
               {
						...
						if (rule.field == "FeedbackDate" && !string.IsNullOrEmpty(rule.value) )
						{	
							if (rule.op == "between")
                            {
                                var datearray = rule.value.Split(new char[] { '-' });
							    ...
							    And(x => SqlFunctions.DateDiff("d", start, x.FeedbackDate) >= 0);
                                And(x => SqlFunctions.DateDiff("d", end, x.FeedbackDate) <= 0);
						    }
						}
         ...
         public  InquiryQuery ByCustomerIdWithfilter(int customerid, IEnumerable<filterRule> filters)
         {
         ...
         public  InquiryQuery ByCompanyIdWithfilter(int companyid, IEnumerable<filterRule> filters)
         {
