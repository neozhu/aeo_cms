...
/// </summary>
    public class QuestionTplService : Service< QuestionTpl >, IQuestionTplService
    {
        private readonly IRepositoryAsync<QuestionTpl> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  QuestionTplService(
          IRepositoryAsync< QuestionTpl> repository,
                ...
                public async Task ImportDataTableAsync(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "QuestionTpl" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到QuestionTpl对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                ...
                {
                    var item = new QuestionTpl();
                    var questiontpltype = item.GetType();
                    foreach (var field in mapping)
                    {
						...
						{
							var propertyInfo = questiontpltype.GetProperty(field.FieldName);
				...
				public async Task<Stream> ExportExcelAsync(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var questiontpls  = this.Query(new QuestionTplQuery().Withfilter(filters)).OrderBy(n=>n.OrderBy(sort,order)).Select().ToList();
            var datarows = questiontpls .Select(  n => new { 
    ...
    Remark = n.Remark
            ...
            return await NPOIHelper.ExportExcelAsync("题库模板", datarows,expcolopts);
        }
