...
{
/// <summary>
/// File: CustomerInvoiceService.cs
...
/// </summary>
    public class CustomerInvoiceService : Service< CustomerInvoice >, ICustomerInvoiceService
    {
        private readonly IRepositoryAsync<CustomerInvoice> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  CustomerInvoiceService(
          IRepositoryAsync< CustomerInvoice> repository,
            ...
            : base(repository)
        {
            ...
            this.logger = logger;
                 ...
                 public async  Task<IEnumerable<CustomerInvoice>> GetByCustomerIdAsync(int  customerid) => await repository.GetByCustomerIdAsync(customerid);
                ...
                public async Task ImportDataTableAsync(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "CustomerInvoice" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到CustomerInvoice对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                    ...
                    )
                {
                    ...
                    foreach (var field in mapping)
                    {
						...
						{
                            var customerinvoicetype = item.GetType();
							var propertyInfo = customerinvoicetype.GetProperty(field.FieldName);
                                                        //关联外键查询获取Id
				...
				public async Task<Stream> ExportExcelAsync(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var customerinvoices  = await this.Query(new CustomerInvoiceQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n=>n.OrderBy(sort,order)).SelectAsync();
            
            var datarows = customerinvoices .Select(  n => new { 
    ...
    CustomerId = n.CustomerId
            ...
            return await NPOIHelper.ExportExcelAsync("发票抬头信息", datarows,expcolopts);
        }
