...
namespace WebApp.Services
{
  ...
  public class CustomerWarehouseService : Service<CustomerWarehouse>, ICustomerWarehouseService
  {
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      return await NPOIHelper.ExportExcelAsync("仓库信息", datarows, expcolopts);
    }
    ...
    public override void ApplyChanges(IEnumerable<ITrackable> entities)
    {
      ...
      {
        var item = (CustomerWarehouse)entity;
        if (item.TrackingState == TrackingState.Added)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "新增仓库信息:" + item.WarehouseName, Auth.GetFullName());
        }
        else if (item.TrackingState == TrackingState.Modified)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "修改仓库信息:" + item.WarehouseName, Auth.GetFullName());
        }
        else if (item.TrackingState == TrackingState.Deleted)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "删除仓库信息:" + item.WarehouseName, Auth.GetFullName());
        }
      }
