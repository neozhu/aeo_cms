...
{
/// <summary>
/// File: CustomerShareService.cs
/// Purpose: Within the service layer, you define and implement 
/// the service interface and the data contracts (or message types).
/// One of the more important concepts to keep in mind is that a service
/// should never expose details of the internal processes or 
/// the business entities used within the application. 
/// Created Date: 2020/7/3 14:05:38
/// Author: neo.zhu
/// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
/// Copyright (c) 2012-2018 All Rights Reserved
/// </summary>
    public class CustomerShareService : Service< CustomerShare >, ICustomerShareService
    {
        private readonly IRepositoryAsync<CustomerShare> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  CustomerShareService(
          IRepositoryAsync< CustomerShare> repository,
            ...
            : base(repository)
        {
            ...
            this.logger = logger;
                 ...
                 public async  Task<IEnumerable<CustomerShare>> GetByCustomerIdAsync(int  customerid) => await repository.GetByCustomerIdAsync(customerid);
                ...
                public async Task ImportDataTableAsync(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "CustomerShare" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到CustomerShare对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                    ...
                    )
                {
                    ...
                    foreach (var field in mapping)
                    {
						...
						{
                            var customersharetype = item.GetType();
							var propertyInfo = customersharetype.GetProperty(field.FieldName);
                                                        //关联外键查询获取Id
				...
				public async Task<Stream> ExportExcelAsync(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var customershares  = await this.Query(new CustomerShareQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n=>n.OrderBy(sort,order)).SelectAsync();
            
            var datarows = customershares .Select(  n => new { 
    ...
    CustomerId = n.CustomerId
            ...
            return await NPOIHelper.ExportExcelAsync("客户分享记录", datarows,expcolopts);
        }
