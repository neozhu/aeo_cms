...
namespace WebApp.Services
{
  ...
  public class InquiryFileService : Service<InquiryFile>, IInquiryFileService
  {
    ...
    public async Task Delete(int[] id, string user)
    {
      ...
      foreach (var item in items)
      {
        ...
        var log = new ActionLog()
        {
          ...
          RefId = item.InquiryId.Value,
          RekKey = item.Inquiry?.InquiryNo,
    ...
    public void AddFile(int ver, int inquiryId, string inquiryNo, HttpPostedFileBase file, string folder, string relpath, string user)
    {
