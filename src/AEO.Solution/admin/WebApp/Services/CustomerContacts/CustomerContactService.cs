...
namespace WebApp.Services
{
  ...
  public class CustomerContactService : Service<CustomerContact>, ICustomerContactService
  {
    ...
    public override void ApplyChanges(IEnumerable<ITrackable> entities)
    {
      ...
      {
        var item = (CustomerContact)entity;
        if (item.TrackingState == TrackingState.Added)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "新增联系人信息:" + item.Name, Auth.GetFullName());
        }
        else if (item.TrackingState == TrackingState.Modified)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "修改联系人信息:" + item.Name, Auth.GetFullName());
        }
