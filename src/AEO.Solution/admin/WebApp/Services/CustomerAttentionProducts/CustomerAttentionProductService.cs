...
{
/// <summary>
/// File: CustomerAttentionProductService.cs
...
/// </summary>
    public class CustomerAttentionProductService : Service< CustomerAttentionProduct >, ICustomerAttentionProductService
    {
        private readonly IRepositoryAsync<CustomerAttentionProduct> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  CustomerAttentionProductService(
          IRepositoryAsync< CustomerAttentionProduct> repository,
            ...
            : base(repository)
        {
            ...
            this.logger = logger;
                 ...
                 public async  Task<IEnumerable<CustomerAttentionProduct>> GetByCustomerIdAsync(int  customerid) => await repository.GetByCustomerIdAsync(customerid);
                ...
                public async Task ImportDataTableAsync(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "CustomerAttentionProduct" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到CustomerAttentionProduct对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                ...
                {
                    var item = new CustomerAttentionProduct();
                    var customerattentionproducttype = item.GetType();
                    foreach (var field in mapping)
                    {
						...
						{
							var propertyInfo = customerattentionproducttype.GetProperty(field.FieldName);
                                                        //关联外键查询获取Id
				...
				public async Task<Stream> ExportExcelAsync(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var customerattentionproducts  = await this.Query(new CustomerAttentionProductQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n=>n.OrderBy(sort,order)).SelectAsync();
            
            var datarows = customerattentionproducts .Select(  n => new { 
    ...
    CustomerName = n.CustomerName
            ...
            return await NPOIHelper.ExportExcelAsync("关注产品信息", datarows,expcolopts);
        }
