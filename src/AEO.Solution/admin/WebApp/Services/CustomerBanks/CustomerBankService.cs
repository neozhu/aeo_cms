...
namespace WebApp.Services
{
  ...
  /// </summary>
  public class CustomerBankService : Service<CustomerBank>, ICustomerBankService
  {
    private readonly IActionLogService actionLogService;
    private readonly IRepositoryAsync<CustomerBank> repository;
    private readonly IDataTableImportMappingService mappingservice;
    private readonly NLog.ILogger logger;
    public CustomerBankService(
      IActionLogService actionLogService,
      IRepositoryAsync<CustomerBank> repository,
    ...
    public async Task<IEnumerable<CustomerBank>> GetByCustomerIdAsync(int customerid) => await repository.GetByCustomerIdAsync(customerid);
    ...
    public async Task ImportDataTableAsync(DataTable datatable, string username)
    {
      var mapping = await this.mappingservice.Queryable()
                        .Where(x => x.EntitySetName == "CustomerBank" &&
                           ( x.IsEnabled == true || ( x.IsEnabled == false && x.DefaultValue != null ) )
                           ).ToListAsync();
      if (mapping.Count == 0)
      {
        throw new KeyNotFoundException("没有找到CustomerBank对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
      }
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      var customerbanks = await this.Query(new CustomerBankQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n => n.OrderBy(sort, order)).SelectAsync();

      var datarows = customerbanks.Select(n => new
      {
    ...
    public override void ApplyChanges(IEnumerable<ITrackable> entities)
    {
      foreach (var entity in entities)
      {
        var item = (CustomerBank)entity;
        if (item.TrackingState == TrackingState.Added)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "新增开户信息:" + item.Bank, Auth.GetFullName());
        }
        else if (item.TrackingState == TrackingState.Modified)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "修改开户信息:" + item.Bank, Auth.GetFullName());
        }
        else if (item.TrackingState == TrackingState.Deleted)
        {
          this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "删除开户信息:" + item.Bank, Auth.GetFullName());
        }
      }
      base.ApplyChanges(entities);
    }
  }
}