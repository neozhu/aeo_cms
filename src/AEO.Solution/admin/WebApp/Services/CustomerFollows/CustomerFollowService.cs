...
{
  /// <summary>
  /// File: CustomerFollowService.cs
  /// Purpose: Within the service layer, you define and implement 
  /// the service interface and the data contracts (or message types).
  /// One of the more important concepts to keep in mind is that a service
  /// should never expose details of the internal processes or 
  /// the business entities used within the application. 
  /// Created Date: 2020/8/5 11:12:35
  /// Author: neo.zhu
  /// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
  /// Copyright (c) 2012-2018 All Rights Reserved
  /// </summary>
  public class CustomerFollowService : Service<CustomerFollow>, ICustomerFollowService
  {
    private readonly IActionLogService actionLogService;
    private readonly IRepositoryAsync<CustomerFollow> repository;
    private readonly IDataTableImportMappingService mappingservice;
    private readonly NLog.ILogger logger;
    public CustomerFollowService(
      IActionLogService actionLogService,
      IRepositoryAsync<CustomerFollow> repository,
    ...
    public async Task<IEnumerable<CustomerFollow>> GetByCustomerIdAsync(int customerid) => await repository.GetByCustomerIdAsync(customerid);
    ...
    public async Task ImportDataTableAsync(DataTable datatable, string username)
    {
      var mapping = await this.mappingservice.Queryable()
                        .Where(x => x.EntitySetName == "CustomerFollow" &&
                           ( x.IsEnabled == true || ( x.IsEnabled == false && x.DefaultValue != null ) )
                           ).ToListAsync();
      if (mapping.Count == 0)
      {
        throw new KeyNotFoundException("没有找到CustomerFollow对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
      }
      foreach (DataRow row in datatable.Rows)
      {
        ...
        {
          var item = new CustomerFollow();
          var customerfollowtype = item.GetType();
          foreach (var field in mapping)
          {
            ...
            {
              var propertyInfo = customerfollowtype.GetProperty(field.FieldName);
              //关联外键查询获取Id
    ...
    public async Task<Stream> ExportExcelAsync(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      var customerfollows = await this.Query(new CustomerFollowQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n => n.OrderBy(sort, order)).SelectAsync();

      var datarows = customerfollows.Select(n => new
      {
      ...
      return await NPOIHelper.ExportExcelAsync("跟进情况", datarows, expcolopts);
    }
    public async Task Delete(int[] id)
    {
      ...
      {
        this.actionLogService.Log(item.CustomerId, item.CustomerCode, "编辑", "删除跟进状态:" + item.Status, Auth.GetFullName());
        this.Delete(item);
      }

    }
    public override void Insert(CustomerFollow entity)
    {
      this.actionLogService.Log(entity.CustomerId, entity.CustomerCode, "编辑", "新增跟进状态:" + entity.Status, Auth.GetFullName());
      base.Insert(entity);
    }
  }
}