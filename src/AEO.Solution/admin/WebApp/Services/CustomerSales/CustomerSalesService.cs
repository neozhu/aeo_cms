...
{
/// <summary>
/// File: CustomerSalesService.cs
/// Purpose: Within the service layer, you define and implement 
/// the service interface and the data contracts (or message types).
/// One of the more important concepts to keep in mind is that a service
/// should never expose details of the internal processes or 
/// the business entities used within the application. 
/// Created Date: 2020/8/5 11:18:32
/// Author: neo.zhu
/// Tools: SmartCode MVC5 Scaffolder for Visual Studio 2017
/// Copyright (c) 2012-2018 All Rights Reserved
/// </summary>
    public class CustomerSalesService : Service< CustomerSales >, ICustomerSalesService
    {
        private readonly IRepositoryAsync<CustomerSales> repository;
		private readonly IDataTableImportMappingService mappingservice;
        private readonly NLog.ILogger logger;
        public  CustomerSalesService(
          IRepositoryAsync< CustomerSales> repository,
            ...
            : base(repository)
        {
            ...
            this.logger = logger;
                 ...
                 public async  Task<IEnumerable<CustomerSales>> GetByCustomerIdAsync(int  customerid) => await repository.GetByCustomerIdAsync(customerid);
                ...
                public async Task ImportDataTableAsync(DataTable datatable,string username)
        {
            var mapping = await this.mappingservice.Queryable()
                              .Where(x => x.EntitySetName == "CustomerSales" && 
                                 (x.IsEnabled == true  || (x.IsEnabled == false &&  x.DefaultValue != null))
                                 ).ToListAsync();
            if (mapping.Count == 0)
            {
                throw new KeyNotFoundException("没有找到CustomerSales对象的Excel导入配置信息，请执行[系统管理/Excel导入配置]");
            }
            foreach (DataRow row in datatable.Rows)
            {
                ...
                {
                    var item = new CustomerSales();
                    var customersalestype = item.GetType();
                    foreach (var field in mapping)
                    {
						...
						{
							var propertyInfo = customersalestype.GetProperty(field.FieldName);
                                                        //关联外键查询获取Id
				...
				public async Task<Stream> ExportExcelAsync(string filterRules = "",string sort = "Id", string order = "asc")
        {
            ...
            var customersales  = await this.Query(new CustomerSalesQuery().Withfilter(filters)).Include(p => p.Customer).OrderBy(n=>n.OrderBy(sort,order)).SelectAsync();
            
            var datarows = customersales .Select(  n => new { 
    ...
    CustomerName = n.CustomerName
            ...
            return await NPOIHelper.ExportExcelAsync("业务员记录", datarows,expcolopts);
        }
