...
namespace WebApp.Controllers
{
  ...
  public class CompaniesController : Controller
  {
    ...
    {
      var list = await getCombo2Data();
      return Json(list, JsonRequestBehavior.AllowGet);
    }
    private async Task<IEnumerable<CompanyComboItem>> getCombo2Data()
    {
      var list = new List<CompanyComboItem>();
      var root = await this.companyService.Queryable().Where(x => x.ParentId == null)
           .Select(x => new
           {
             x.Id,
             x.Name
      ...
      {
        var i = 1;
        var childid = await this.getOwnerChildrenId(top.Id);
        list.Add(new CompanyComboItem()
        {
          includes = string.Join(",", childid.ToArray()),
          id = top.Id,
          text = top.Name,
          level = 0
        });
        await recursioncomboitem(list, top.Id, i++);
      }
      return list;

    }
    private async Task recursioncomboitem(List<CompanyComboItem> list, int? parentid, int i)
    {
      var children = await this.companyService.Queryable().Where(x => x.ParentId == parentid
           )
          .Select(x => new
          {
            x.Id,
      ...
      foreach (var child in children)
      {
        var childid = await this.getOwnerChildrenId(child.Id);
        list.Add(new CompanyComboItem()
        {
          includes = string.Join(",", childid.ToArray()),
          id = child.Id,
        ...
        await recursioncomboitem(list, child.Id, i);
      }
      //if (children.Count > 0)
      //{
      //  item.children = children.ToArray();
    ...
    //递归获取当前id下包含的所有子id
    private async Task<IEnumerable<int>> getOwnerChildrenId(int id)
    {
      var result = new List<int>();
      result.Add(id);
      var children = await this.companyService.Queryable().Where(x => x.ParentId == id)
        .Select(x => x.Id).ToArrayAsync();
      foreach (var child in children)
      {
        result.Add(child);
        var any = await this.companyService.Queryable().Where(x => x.ParentId == child).AnyAsync();
        if (any)
        {
          await recursionchildid(result, child);
        }
      }
      return result;
    }
    private async Task recursionchildid(List<int> result, int parentid)
    {
      var children = await this.companyService.Queryable().Where(x => x.ParentId == parentid)
        .Select(x => x.Id).ToArrayAsync();
      foreach (var child in children)
