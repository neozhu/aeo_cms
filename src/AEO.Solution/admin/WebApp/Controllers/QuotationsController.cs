   ...
   CustomerCode = n.Customer?.CustomerCode,
    ...
    ApprovedDate = n.ApprovedDate?.ToString("yyyy-MM-dd HH:mm:ss")
        ...
        {    
            var filters = JsonConvert.DeserializeObject<IEnumerable<filterRule>>(filterRules);
			    var pagerows = (await this.quotationService
						               .Query(new QuotationQuery().ByCompanyIdWithfilter(companyid,filters)).Include(q => q.Company).Include(q => q.Customer)
							           .OrderBy(n=>n.OrderBy(sort,order))
							           .SelectPageAsync(page, rows, out var totalCount))
                                       .Select(  n => new { 
   ...
   CustomerCode = n.Customer?.CustomerCode,
    ...
    ApprovedDate = n.ApprovedDate?.ToString("yyyy-MM-dd HH:mm:ss")
        ...
        {    
            var filters = JsonConvert.DeserializeObject<IEnumerable<filterRule>>(filterRules);
			    var pagerows = (await this.quotationService
						               .Query(new QuotationQuery().ByCustomerIdWithfilter(customerid,filters)).Include(q => q.Company).Include(q => q.Customer)
							           .OrderBy(n=>n.OrderBy(sort,order))
							           .SelectPageAsync(page, rows, out var totalCount))
                                       .Select(  n => new { 
