...
namespace WebApp.Controllers
{
  ...
  public class ProductsController : Controller
  {
    ...
    public async Task<ActionResult> Create(CreateProductViewModel product)
    {
      if (ModelState.IsValid)
      {
    ...
    //修改添加图片
    [HttpPost]
    public async Task<JsonResult> AddPrictures(int id, string[] fileId) {
      await this.productService.AddPrictures(id,fileId);
      await this.unitOfWork.SaveChangesAsync();
      return Json(new { success = true }, JsonRequestBehavior.AllowGet);
    }
    public async Task<JsonResult> DeleteFile(string id) {
     await this.productService.DeleteFile(id);
      await this.unitOfWork.SaveChangesAsync();
      return Json(new { success = true }, JsonRequestBehavior.AllowGet);
    }
    //新增对象初始化
