...
namespace Aim.Portal.Data
{
    ...
    public class DataImportService
    {
        ...
        public static void DoImportTemplateFileChanged(SysDataImportTemplate ent)
        {
            ...
            ImportTemplateStructure its = itp.GetStructure() as ImportTemplateStructure;
        ...
        public static void ImportDataByTemplateCode(string tmplCode, string fileFullName)
        {
            ...
            ImportData(its, filePath, null, null);
        }
        ...
        /// <param name="tmplCode">模板编码</param>
        /// <param name="fileFullName"></param>
        public static void ImportDataByTemplateCode(string tmplCode, string fileFullName, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
            SysDataImportTemplate tmplEnt = SysDataImportTemplate.FindFirstByProperties("Code", tmplCode);
            ...
            ImportData(its, filePath, OnImporting, OnImported);
        }
        ...
        {
            ImportData(config, fileFullName, null, null);
        }
        ...
        /// <param name="template"></param>
        /// <param name="fileFullName"></param>
        public static void ImportData(string config, string fileFullName, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
        ...
        {
            ImportData(struc, filePath, null, null);
        }
        ...
        /// <param name="filePath"></param>
        /// <param name="blockSize"></param>
        public static void ImportData(ImportTemplateStructure struc, string filePath, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
            SqlConnection sqlConnection = DataHelper.GetCurrentDbConnection() as SqlConnection;

            ImportData(struc, struc.DefaultGroup.PropertyNode.Target, sqlConnection, filePath, 0, OnImporting, OnImported);
        }
        ...
        /// <param name="filePath"></param>
        public static void ImportData(ImportTemplateStructure struc, string targetTable, string filePath, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
            SqlConnection sqlConnection = DataHelper.GetCurrentDbConnection() as SqlConnection;

            ImportData(struc, targetTable, sqlConnection, filePath, 0, OnImporting, OnImported);
        }
        ...
        /// <param name="filePath"></param>
        /// <param name="blockSize"></param>
        public static void ImportData(ImportTemplateStructure struc, string targetTable, SqlConnection sqlConnection, string filePath, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
            int blockSize = 0;
            ...
            ImportData(struc, targetTable, sqlConnection, filePath, blockSize, OnImporting, OnImported);
        }
        ...
        public static void ImportData(ImportTemplateStructure struc, string targetTable, SqlConnection sqlConnection, string filePath, int blockSize, Func<DataTable, bool> OnImporting, Action<DataTable> OnImported)
        {
            ...
            foreach (DataTable tdt in dts)
            {
                try
                {
                    ...
                    DataTable ftdt = FillTargetDataTable(tdt, targetTable, sqlConnection);

                    bool flag = true;
                    if (OnImporting != null)
                    {
                        flag = OnImporting(ftdt);
                    }

                    if (flag)
                    {
                        DataHelper.CopyDataToDatabase(ftdt, sqlConnection, targetTable);

                        successAmount += ftdt.Rows.Count;
                    }
        ...
        /// <summary>
        /// 将dt中对应数据保存到targetTable的Schema DataTable中
        /// </summary>
        /// <param name="targetTable"></param>
        /// <param name="dt"></param>
        /// <returns></returns>
        private static DataTable FillTargetDataTable(DataTable dt, string targetTable, IDbConnection conn)
        {
            DataTable tdt = DataHelper.GetDataSchema(targetTable, conn);

            foreach (DataColumn col in dt.Columns)
            {
                if (!tdt.Columns.Contains(col.ColumnName))
                {
            ...
            {
                DataRow tdrow = tdt.NewRow();

                foreach (DataColumn col in dt.Columns)
                {
                    // 去除空格值
                    if (drow[col] != null && drow[col].ToString().Trim().Length > 0)
                    {
                        tdrow[col.ColumnName] = drow[col];
                    }
                }
