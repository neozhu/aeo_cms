...
namespace Aim.Portal
{
                ...
                try
                {
                    lock (uclslocker)
                    {
                        ...
                        else if (!String.IsNullOrEmpty(CurrentUserSID) && !String.IsNullOrEmpty(CurrentUserInfo.LoginName))
                        {
                            ...
                            if (!uc.ExtData.ContainsKey("CompanyId"))
                            {
                                if (HttpContext.Current.Request.Cookies["companyid"] != null)
                                {
                                    uc.ExtData["CompanyId"] = HttpContext.Current.Request.Cookies["companyid"].Value;
                                    uc.ExtData["CompanyName"] = System.Web.HttpUtility.UrlDecode(HttpContext.Current.Request.Cookies["companyname"].Value);
                                }
                            }
    ...
    public class SystemContext
    {
        ...
        {
            applications = new ReadOnlyCollection<SysApplication>(SysApplication.FindAll(Expression.Sql("Status<>0 or Status is null")));
