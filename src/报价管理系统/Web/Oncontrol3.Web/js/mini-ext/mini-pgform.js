...
var UPLOAD_PAGE_URL2 = "/CommonPages/File/Upload2.aspx";
...
//表单必填项label的前缀,默认为"*";
var FIELD_REQUIRED_PREFIX = "*";
//表单必填项label的文本颜色默认为"#900";
var FIELD_REQUIRED_LABELCOROR = "#900";
//查询订阅页面路径
var SUBSCRIBE_URL = "/CommonPages/SysSubscribeEdit.aspx";
...
function FormValidationBind(controlId, funcSuccess, funcfail) {
    ...
    if ($("[class*=validate]").length > 0) {
        var frmId = AimFrm.id;
        $("#" + frmId).validationEngine("attach", { promptPosition: 'topLeft' });
        //jQuery("#" + frmId).validationEngine({
        //    promptPosition: 'inline',
        //    onSuccess: funcSuccess,
        //    onFailure: funcfail
        //});
        $("#" + controlId).click(function() {
            if ($("#" + frmId).validationEngine('validate')) {
                funcSuccess();
            } else {
                if (funcfail && typeof funcfail == "function") {
                    funcfail();
...
(function() {
    function AimForm(htmele) {
        ...
        function dataBind(dt) {
            ...
            if (dt) {
                ...
                //add by cc begin
                if (mode != "c" && mode != "cs" && mode != "cp" && AimState["frmdata"]) {
                    ...
                    $("input[type=checkbox]").each(function() {
                        if (this.value == AimState["frmdata"][this.id]) {
                            this.checked = true;
                        }
                    });
                }
                ...
                /*
                *按钮值不需要设置   huangkai 原始代码$(":input")修改为$(":input:not(:button)")
                *
                */
                $(":input:not(:button,:radio,:checkbox)").each(function() {
                    ...
                    if (mode == "c" || mode == "cs") {
                        /*
                        *基于miniui的选人控件会在input的id后面加上$value
                        */

                        defVal = $("[id='" + cname + "']").val() || defVal;
                    }
    ...
    // 提交数据
    $.submitData = $.extend(true, Aim.Form, {
        submitData: function(frmdata, action, dt, url, onProcFinished, onProcExcept) {
...
(function() {
    function AimSelect(htmele) {
        ...
        var relateId = htmlele.attr("relateid");
        var formatter = null;
        var isSelected = false;
        var Readonly = htmlele.attr("readonly");
        var Required = htmlele.attr("required");
        var emptyText = htmlele.attr("emptyText") || "请选择...";
        var DefaultValue = htmlele.attr("defaultvalue") || "";

        var _OldIndex = null;
        var _OldHandle = null;

        function init() {
            if (Required && Required.toLowerCase() == "true" || Required == "required") {
                Required = true;
            } else {
                Required = false;
            }
        ...
        function bindData(jsonVal) {
            ...
            if (DefaultValue != "") {
                htmlele.children().each(function() {
...
(function() {
    function AimDate(htmele) {
        var htmlele = typeof (htmele) == "object" ? $("#" + htmele.id) : $("#" + htmele);
        var datefmt = htmlele.attr("datefmt");
        var minDateId = htmlele.attr("mindatedom");
        var maxDateId = htmlele.attr("maxdatedom");
        var minDate = htmlele.attr("mindate");
        var maxDate = htmlele.attr("maxdate");
        var isSelected = false;
        var Readonly = htmlele.attr("readonly");
        var AfterPicked = htmlele.attr("afterpicked");
        var onclick = htmlele.attr("onclick");

        function init() {
            var dmindate = '1970-01-01';
            var dmaxdate = '2020-12-31';
            htmlele.addClass("Wdate");
            var defaultFmt = "yyyy-MM-dd";

            var options = {};
            options.dateFmt = datefmt ? datefmt : defaultFmt;
            if (minDateId || minDate) {
                if (minDateId) {
                    options.minDate = "#F{$dp.$D(\'" + minDateId + "\')||$dp.$DV(\'" + dmindate + "\')}";
                }
                else if (minDate) {
            ...
            if (maxDateId || maxDate) {
                if (maxDateId) {
                    options.maxDate = "#F{$dp.$D(\'" + maxDateId + "\')||$dp.$DV(\'" + dmaxdate + "\')}";
                }
                else if (maxDate) {
            ...
            if (AfterPicked) {
                options.onpicked = eval('(' + AfterPicked + ')');
            }
            if (!onclick) {
                htmlele.click(function() {
                    WdatePicker(options);
                });
            }
        }
...
(function() {
    function AimPopUp(htmele) {
        ...
        function init() {
            deptsel = new mini.ux.AimComSelect({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                nameField: htmele.getAttribute("namefield"),

                valueField: htmele.getAttribute("valuefield"),
                textField: htmele.getAttribute("textfield"),

                afterSelect: eval(htmele.getAttribute("afterselect")),
                width: htmele.getAttribute("width") ? htmele.getAttribute("width") : "100%",
                height: htmele.getAttribute("height") ? htmele.getAttribute("height") : "100%",
                url: htmele.getAttribute("url"),
                title: htmele.getAttribute("title")
            });

            if (AimState["frmdata"]) {
                if (AimState["frmdata"][htmele.getAttribute("idfield")])
                    deptsel.setValue(AimState["frmdata"][htmele.getAttribute("idfield")]);
                if (AimState["frmdata"][htmele.getAttribute("namefield")])
                    deptsel.setText(AimState["frmdata"][htmele.getAttribute("namefield")]);
            }
        }
        ...
        function setValue(val) {
            deptsel.setValue(val)
        }
        function setText(val) {
            deptsel.setText(val);
        }
        function setReadOnly(bool) {
            var editor = mini.get(htmele.id);
            if (editor != null) {
...
(function() {
    function AimFile(htmele) {
        ...
        var eleSpan, fileLinkSpan, fileBtnSpan;
        ...
        function init() {
            ...
            if (htmlele.parent().prop("tagName").toLowerCase() == "span") {
...
(function() {
    function AimFile2(htmele) {
        ...
        var AllowThumbnail = false; // 是否允许显示缩略图
        var DoCheck = htmlele.attr("DoCheck") || null; // 上传前检查方法
        var UploadPage = htmlele.attr("UploadPage") || UPLOAD_PAGE_URL2;
        var DownloadPage = htmlele.attr("DownloadPage") || DOWNLOAD_PAGE_URL;
        ...
        var UploadStyle = "<div id='showDialog' style=\"background-color:rgb(233, 240, 252); margin:0px;display:none;text-align: center;vertical-align: center\" >"
        ...
        var filedWidth = htmlele.css("width") || "100%";
        var SingleStructure = UploadStyle + " <table style='border:0px; width:" + filedWidth + "; font-size:12px;margin-left:4px;height:30px;'><tr>"
            + "<td style='width:*; vertical-align:top; border-color:#8FAACF; padding:2px;' class='aim-ctrl-file'><span id='" + FileInputFieldID + "' style='width:100%' /></td>"
            + "<td style='width:120px; border:0px; padding:0px;' align='center'><span id='" + FileBtnSpanID + "' class='aim-ctrl-file-button-span'>"
            + "<div class='button-group'>"
            + "<a id='" + BtnFileAddID + "' class='button'>上传</a>"
            + "<a id='" + BtnFileClrID + "' class='button'>清空</a>"
            + "</div>"
            + "</span></td></tr></table>";
        ...
        $('#showDialog').dialog({
            autoOpen: false,
            width: 428,
            height: 320, // (window.screen.availHeight - 500),//415,
            modal: true,
            title: '上传附件',
            buttons: {
                '完成': function() {
                    var rtn = window.frames["fileframe"].document.getElementById("files").value;
                    if (rtn == "0") {
                    ...
                    if (rtn == "") {
                        if (confirm("您没有上传文件！是否关闭窗口")) {
                            window.frames["fileframe"].document.getElementById("files").value = "";
                            $(this).dialog("close");
                            return;
                        } else {
                            return;
        ...
        function init() {
            ...
            btnFileAdd.click(function() {
                ...
                $("#showDialog").dialog("option", "draggable", false);
                $("#id").dialog("option", "bgiframe", true);
    ...
    // Aim.File2
    $.extend(true, window, { Aim: { File2: AimFile2} });
} ());
...
(function() {
    function AimDept(htmele) {
        var htmlele = typeof (htmele) == "object" ? $("#" + htmele.id) : $("#" + htmele);
        var deptsel;

        function init() {
            deptsel = new mini.ux.AimDeptSelect({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                nameField: htmele.getAttribute("namefield"),
                allowInput: htmele.getAttribute("allowinput") == "true" ? true : false,
                afterSelect: eval(htmele.getAttribute("afterselect")),
                width: htmele.getAttribute("width") ? htmele.getAttribute("width") : "100%",
                height: htmele.getAttribute("height") ? htmele.getAttribute("height") : "100%"
            });
    ...
    $.extend(true, window, { Aim: { Dept: AimDept} });
} ());
...
(function() {
    function AimUser(htmele) {
        ...
        function init() {
            deptsel = new mini.ux.AimUserSelect({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                popParam: htmele.getAttribute("popparam"),
                popUrl: htmele.getAttribute("popurl"),
                nameField: htmele.getAttribute("namefield"),
                afterSelect: eval(htmele.getAttribute("afterselect")),
                width: htmele.getAttribute("width") ? htmele.getAttribute("width") : "100%",
                height: htmele.getAttribute("height") ? htmele.getAttribute("height") : "100%",
                multiSelect: htmele.getAttribute("multi") == "true" ? true : false
            });
        ...
        function getValue() {
            return deptsel.getValue();
        }
        function getText() {
            return deptsel.getText();
        }
...
(function() {
    function AimDialog(htmele) {
        ...
        function init() {
            ...
            else if (typeof (mini) == "object" && typeof (mini.MessageBox) == "object") {
                mode = "mini";
...
(function() {
    function AimCustom(htmele) {
        var htmlele = typeof (htmele) == "object" ? $("#" + htmele.id) : $("#" + htmele);
        var customsel;

        function init() {
            customsel = new mini.ux.AimCustomSelect({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                nameField: htmele.getAttribute("namefield"),
                disColName: htmele.getAttribute("discolname"),
                popUrl: htmele.getAttribute("popurl"),
                showSelBtn: htmele.getAttribute("showselbtn"),

                required: htmele.getAttribute("required"),

                valueField: htmele.getAttribute("valuefield"),
                textField: htmele.getAttribute("textfield"),
                emptyTitle: htmele.getAttribute("emptytitle"),

                selSql: htmele.getAttribute("selsql"),
                selColName: htmele.getAttribute("selcolname"),
                popWidth: htmele.getAttribute("popwidth"),
                selTable: htmele.getAttribute("seltable"),
            ...
            if (AimState["frmdata"]) {
                if (AimState["frmdata"][htmele.getAttribute("idfield")])
                    customsel.setValue(AimState["frmdata"][htmele.getAttribute("idfield")]);
                if (AimState["frmdata"][htmele.getAttribute("namefield")])
                    customsel.setText(AimState["frmdata"][htmele.getAttribute("namefield")]);
            }
        }
        ...
        function getValue() {
            return customsel.getValue();
        }
        function getText() {
            return customsel.getText();
        }

        function validate() {
            return customsel.validate();
        }

        function setValue(val) {
            customsel.setValue(val);
        }
        function setText(val) {
            customsel.setText(val);
        }
        function setReadOnly(bool) {
            if (bool) {
                customsel.disable();
    ...
    $.extend(true, window, { Aim: { Custom: AimCustom} });
} ());
//------------------------Aim Custom 结束------------------------//
...
(function() {
    function AimMCombox(htmele) {
        var htmlele = typeof (htmele) == "object" ? $("#" + htmele.id) : $("#" + htmele);
        var multicom;

        function init() {
            multicom = new mini.ux.AimMultiCombox({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                nameField: htmele.getAttribute("namefield"),
                multiSelect: true,
                ...
                valueField: htmele.getAttribute("valuefield"),
                textField: htmele.getAttribute("textfield"),
                data: htmele.getAttribute("enum") || htmele.getAttribute("enumdata") || htmele.getAttribute("data"),
            ...
            if (AimState["frmdata"]) {
                if (AimState["frmdata"][htmele.getAttribute("idfield")])
                    multicom.setValue(AimState["frmdata"][htmele.getAttribute("idfield")]);
                if (AimState["frmdata"][htmele.getAttribute("namefield")])
                    multicom.setText(AimState["frmdata"][htmele.getAttribute("namefield")]);
            }
        }
        ...
        function setData(datas) {
            multicom.setData(datas);
        }

        function setValue(val) {
            multicom.setValue(val)
        }
        function setText(val) {
            multicom.setText(val)
        }
        function setReadOnly(bool) {
            var editor = mini.get(htmele.id);
            if (editor != null) {
    ...
    $.extend(true, window, { Aim: { MCombox: AimMCombox} });
} ());
...
(function() {
    function AimTCombox(htmele) {
        var htmlele = typeof (htmele) == "object" ? $("#" + htmele.id) : $("#" + htmele);
        var multicom;

        function init() {
            tcom = new mini.ux.AimTreeCombox({
                applyTo: htmele.id,
                idField: htmele.getAttribute("idfield"),
                nameField: htmele.getAttribute("namefield"),
                multiSelect: htmele.getAttribute("multi") == "true" ? true : false,
                ...
                valueField: htmele.getAttribute("valuefield"),
                textField: htmele.getAttribute("textfield"),
                parentField: htmele.getAttribute("parentfield"),
                checkRecursive: htmele.getAttribute("checkrecursive") == "true" ? true : false,
                showFolderCheckBox: htmele.getAttribute("showfoldercheckbox") == "true" ? true : false,
                expandOnLoad: htmele.getAttribute("expandonload") == "true" ? true : false,
                showClose: htmele.getAttribute("showclose") == "true" ? true : false,
                autoCheckParent: htmele.getAttribute("autocheckparent") == "true" ? true : false,
                data: htmele.getAttribute("enum") || htmele.getAttribute("enumdata"),
            ...
            if (AimState["frmdata"]) {
                if (AimState["frmdata"][htmele.getAttribute("idfield")])
                    tcom.setValue(AimState["frmdata"][htmele.getAttribute("idfield")]);
                if (AimState["frmdata"][htmele.getAttribute("namefield")])
                    tcom.setText(AimState["frmdata"][htmele.getAttribute("namefield")]);
            }
        }
    ...
    $.extend(true, window, { Aim: { TCombox: AimTCombox} });
} ());
