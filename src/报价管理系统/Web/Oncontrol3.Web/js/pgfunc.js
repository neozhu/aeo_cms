...
$().ready(function() {
    ...
    //增加语言切换层
    //    if (window.parent.document.getElementById("divforextension") == null && window.location.href.indexOf("Top.aspx") < 0) {
    //        var boarddiv = "<div id='divforextension' style='background-color:;width:200px;height:25px;line-height:25px;vertical-align:middle;font-size:12px;'>";
    //        boarddiv += "<a href='#' onclick=changeLanguage(0)>中文</a>&nbsp;|&nbsp;<a href='#' onclick=changeLanguage(1)>English</a>";
    //        boarddiv += "&nbsp;加载耗时:" + Math.round(1000*AimState["TTSECONDRENDER"])/1000+"秒";
    //        boarddiv += "</div>";
    //        $(document.body).append(boarddiv);
    //        $("#divforextension").floatdiv("middlebottom");
    //    }
});
function changeLanguage(tag) {
    switch (tag) {
        case 0:
        default:
            $.ajaxExec('languageset', { "lang": "" }, function(rtn) {
                window.location.reload();
            });
            break;
        case 1:
            $.ajaxExec('languageset', { "lang": "en" }, function(rtn) {
                window.location.reload();
            });
            break;
    }
        ...
        if (atype && this.id) {
            switch (atype.toLowerCase()) {
                ...
                case "select":
                    ...
                    break;
                case "customerquicksel":
                    actrl = ((typeof (Aim.CustomerQuickSel) == "undefined") ? null : (new Aim.CustomerQuickSel(this)));
                    break;
                case "popup":
                ...
                case "user":
                    ...
                    break;
                case "combo":
                    actrl = ((typeof (Aim.Combo) == "undefined") ? null : (new Aim.Combo(this)));
                    break;
                case "file":
