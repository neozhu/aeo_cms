...
function ExtRequest(params, url, isUpload, callback) {
    if (!Ext.fly('frmDummy')) {
...
function ExtGridDateOnlyRender(val, p, rec) {
    if (val) {
        return $.dateOnly(val);
        //return new Date(val.toString().replace(/-/g, "/")).format("Y/m/d")
    }
    else {
        return "";
    }
...
Ext.ux.grid.AimGridPanel = Ext.extend(Ext.grid.GridPanel, {
    constructor: function(config) {
        ...
        config.sm = config.sm || new Ext.ux.grid.AimCheckboxSelectionModel({ singleSelect: config.singleSelect });
...
Ext.ux.AimSchPanel = Ext.extend(Ext.Panel, {
    constructor: function(config) {
        ...
        //        config.showSchBtn = !(config.showSchBtn == false);// ? false : config.showSchBtn || true;
        //        config.schBtnText = config.schBtnText || '查 询';
        if (config.padding !== 0) {
        ...
        if (!config.layout) {
            ...
            if (config.items && $.isArray(config.items)) {
                if (config.items.length > 0 && config.showSchBtn) {
                    config.items.push({
                        layout: 'column',
                        xtype: 'container',
                        items: [{
                            ...
                            text: config.schBtnText,
                            handler: function() {
                                Ext.ux.AimDoSearch(Ext.getCmp(config.items[0].items[0].items[0].id));
                            }
}]
                        });
                    }
                    ...
                    for (var i = 0, pi = 0; i < config.items.length; i++) {
                        ...
                        if (i == 0 || (tflexcount + tfitem.flex) > pi) {
                            tautoitem = { layout: 'hbox', defaults: { margins: '0 0 0 0' }, listeners: { afterlayout: function(f) { f.el.dom.firstChild.firstChild.style.border = "none" } }, items: [] };
...
Ext.ux.AimDeptSelector = Ext.extend(Ext.ux.form.AimPopup, {
    constructor: function(config) {
        config.popUrl = config.popUrl || "/CommonPages/Select/GrpSelect/MGrpSelect.aspx?rtntype=array&cid=2";
        config.seltype = config.seltype || "single";
        config.popUrl += "&seltype=" + config.seltype;
...
//------列表选择枚举 add by cc begin-----
Ext.ux.AimDeptSelector = Ext.extend(Ext.ux.form.AimPopup, {
    constructor: function(config) {
        config.popUrl = config.popUrl || "/CommonPages/Select/EnumSelect.aspx?rtntype=array&EnumKey=" + config.EnumKey;
...
Ext.reg('aimenumselector', Ext.ux.AimDeptSelector);
//------列表选择枚举 add by cc end-----
...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect, Ext.form.ComboBox, {
    ...
    */
    styleField: null,
    /**
    * @cfg {String} phoneField The underlying data field that will be used to supply additional phonenumber to each item.
    */
    phoneField: null,
    /**
    * @cfg {String} phoneField The underlying data field that will be used to supply additional phonenumber to each item.
    ...
    doTransform: function() {
        ...
        if (!this.store) {
            ...
            this.store = new Ext.data.SimpleStore({
                'id': 0,
                fields: ['value', 'text', 'cls', 'style', 'phone', 'workno'],
                data: d
            });
            Ext.apply(this, {
                valueField: 'value',
    ...
    */
    ...
    addRecord: function(record) {
        ...
        phone = this.phoneField ? record.data[this.phoneField] : '';
        workno = this.worknoField ? record.data[this.worknoField] : '';
        ...
        this.addItemBox(val, display, caption, cls, style, phone, workno);
    ...
    createRecord: function(recordData) {
        if (!this.recordConstructor) {
            ...
            if (this.phoneField) {
                recordFields.push({ name: this.phoneField });
            }
            if (this.worknoField) {
                recordFields.push({ name: this.worknoField });
            }
    ...
    */
    ...
    addItemBox: function(itemVal, itemDisplay, itemCaption, itemClass, itemStyle, phoneVal, worknoVal) {
    ...
    */
    getValueEx: function() {
        ...
        this.items.each(function(item) {
            ...
            if (this.phoneField) {
                newItem[this.phoneField] = item.phone || '';
            }
            if (this.worknoField) {
                newItem[this.worknoField] = item.workno || '';
            }
            ret.push(newItem);
        }, this);
        return ret;
...
var userselstore = new Ext.data.JsonStore({
    id: 'UserID',
    root: 'rows',
    fields: [{ name: 'UserID' }, { name: 'Name' }, { name: 'LoginName' }, { name: 'WorkNo' }, { name: 'Status' }, { name: 'Phone'}],
...
Ext.ux.form.AimUser = Ext.extend(Ext.ux.form.SuperBoxSelect, {
    ...
    '<tpl for="."><div class="x-combo-list-item" title="{WorkNo}">{Name}{WorkNo}</div></tpl>'
    ),
    anchor: '100%',
    mode: 'remote',
    displayField: 'Name',
    ...
    constructor: function(config) {
        ...
        this.on("additem", function(bs, v, record) {
            if (typeof (this.popAfter) == "function") {
                if (bs.seltype == "single") {
        ...
        this.on("removeitem", function(bs, v, record) {
            if (bs.seltype == "single") {
                if (this.removeItem) {
                    this.removeItem.call(bs, v, {});
                    bs.reset();
                }
            }
            else {
                if (typeof (this.popAfter) == "function") {
                    this.removeItem.call(bs, v, record);
                }
            }
        })
    },
        ...
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    try {
                        this.setValueEx([{ UserID: rtnval.data.UserID || rtnval.data[0].UserID, Name: rtnval.data.Name || rtnval.data[0].Name, Phone: rtnval.data.Phone || rtnval.data[0].Phone, WorkNo: rtnval.data.WorkNo || rtnval.data[0].WorkNo}]);
                    } catch (e) { }
                }
                else {
                    for (var i = 0; i < rtnval.data.length; i++) {
                        this.addItem({ UserID: rtnval.data[i].UserID, Name: rtnval.data[i].Name, Phone: rtnval.data[i].Phone, WorkNo: rtnval.data[i].WorkNo });
                    }
                }
            }
        }
...
var idstr = "store" + Math.floor(Math.random() * 24);
var customerselstore = new Ext.data.JsonStore({
    id: idstr,
    root: 'rows',
    fields: [{ name: 'Id' }, { name: 'Code' }, { name: 'Name'}],
    url: '/commonpages/data/customerdata.aspx',
    baseParams: { cmd: 'GetCustomer' }
});
Ext.ux.form.AimCustomerQuickSel = Ext.extend(Ext.ux.form.SuperBoxSelect, {
    ...
    extParams: "",
    constructor: function(config) {
        this.popUrl = config.PopUrl; //"/CommonPages/Select/CustomerSelect.aspx?rtntype=array";
        this.seltype = config.seltype || "single";
        this.popUrl += "&seltype=" + this.seltype;
        this.popParam = config.popParam;
        this.popStyle = config.popStyle || "dialogWidth:750px; dialogHeight:550px; scroll:yes; center:yes; status:no; resizable:yes;";
        ...
        Ext.ux.form.AimCustomerQuickSel.superclass.constructor.call(this, Ext.apply(config, {}));
        this.on("beforeadditem", function(bs, v) {
            if (bs.seltype == "single") {
                bs.reset();
                //bs.addItem(v);
    ...
    onTriggerClick: function() {
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    this.setValueEx([{ Id: rtnval.data.Id || rtnval.data.ID, Name: rtnval.data.Name, Code: rtnval.data.Code}]);
                }
                else {
                    for (var i = 0; i < rtnval.data.length; i++) {
                        this.addItem({ Id: rtnval.data[i].Id || rtnval.data[i].ID, Name: rtnval.data[i].Name, Code: rtnval.data[i].Code });
                    }
                }
            }
        }
...
Ext.reg('aimcustomerquicksel', Ext.ux.form.AimCustomerQuickSel);
