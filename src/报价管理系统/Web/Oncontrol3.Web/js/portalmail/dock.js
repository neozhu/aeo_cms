    ...
    this.method = {
        ...
        */
        create: function() {
            ...
            // 创建开始按钮
            handler.dockStart = $('<div  style="cursor:pointer"></div>').addClass('neter-dock-start-button-container')
            ...
            defaults.maskFlash
                && handler.dock.append($('<iframe frameborder=0 src="' + Neter.path() + 'blank.html"></iframe>').addClass('neter-dock-iframe'));
        ...
        */
        bindEvents: function() {
            ...
            handler.dock
            //删除事件扩展
            .on('click', '.neter-dock-list-label', function(event) {
                if (!confirm("您确定要移除该快捷方式？")) return;
                var itemEvent = defaults.itemEvent,
                    options = method.getOptions(this.parentElement); //method.getOptions(this, $(this).data('level') == 2 ? handler.subApps : null);
                if (typeof itemEvent === 'function'
                    && itemEvent.call(this, _this, Neter.apply({}, options, ['app4full', 'app4list']), defaults.currentModel, event, this.id) !== false) {
                    _this.remove(options.name);
                } else {
                }
                event.stopPropagation();
            })
        ...
        */
        insert: function(index, app) {
            ...
            item.app4list = $('<div></div>').addClass('neter-dock-list-item')
                ...
                .append($('<div id=' + app.id + ' title=移除></div>').addClass('neter-dock-list-label').html('一').show())
