...
and t1.service_code = '{2}' ";
                        ...
                        string sql_fee = "select t1.RID, t1.FEE_CODE, t1.FEE_NAME, t1.BJSTATAUS,t1.OTHER_NAME " + sql_common;
                        ...
                        string listpsfsql = string.Format(sql_fee, ver_id, product_code, service_code);
                        IList<EasyDictionary> listpsf = DataHelper.QueryDictList(listpsfsql);
                        // 判断是否有“运输路线”列  为了生成的Excel列数对齐（同一服务，不同计费基础的费目保持列数一致，这需求真特么不合理） 改版，最新版本中无“运输路线”
                        //int colnum = 6;
                        foreach (EasyDictionary dic in listpsf)
                        {
                            if (dic.Get("FEE_NAME") + "" == "手工费目" || dic.Get("FEE_NAME") + "" == "自定义报价")
                            {
                                sql = @"select '' ""序号"",SERVICE_NAME ""服务"",FEE_NAME ""费目"",
                                      FEEUNIT as ""计费单位"",PRICEBZ as ""单价"",'' ""最低收费"",FSFYSM ""费用说明"",BGF ""包干费"" from SQM_BJ_CUSTOMIZE
                                where BJRID='" + dic.Get("RID") + "" + "'";
                                ...
                                string djfsrid = "";
                                string gdzrid = "";
                                string ediclistsql = "select distinct DJFSRID from SQM_FEE_CALC_REF where STATUS = '1' and FEECODE = '" + feecode + "' and (DJFSRID <> '' or DJFSRID is not null)";
                                IList<EasyDictionary> ediclist = DataHelper.QueryDictList(ediclistsql);// DJFSRID不为空
                                string dtdjfswjcsql = "select distinct djfsrid from sqm_fee_pur_ref where djfsrid not in(select djfsrid from sqm_fee_calc_ref where feecode = '" + feecode + "') and feecode = '" + feecode + "'";
                                DataTable dtdjfswjc = DataHelper.QueryDataTable(dtdjfswjcsql);// 定价方式无基础
                                if ((ediclist.Count > 0 && bjfs == "0") || (dtdjfswjc.Rows.Count > 0 && bjfs == "0"))
                                {
                                    foreach (EasyDictionary ed in ediclist)// 遍历定价方式
                                    {
                                        ...
                                        if (gdzlist.Count > 0)
                                        {
                                            ...
                                            foreach (EasyDictionary gdz in gdzlist)
                                            {
                                                ...
                                                if (gdzrid != "")
                                                {
                                                    sql = getFeeSqlNew(ver_id, service_code, product_code, feecode, "", djfsrid, gdzrid, dic.Get("RID").ToString());
                                                    if (sql != "")
                                                    {
                                                        DrawExcel(cells, ref rowIndex, sql, ref ifTitle, 0);
                                                    }
                                                    //List<string> sqll=getFeeSqlTry(ver_id, service_code, product_code, feecode, "", djfsrid, gdzrid);
                                                    //foreach (string sqls in sqll)
                                                    //{
                                                    //    DrawExcel(cells, ref rowIndex, sqls, ref ifTitle, 0);
                                                    //}
                                                }
                                                else // 无高低值
                                                {
                                                    sql = getFeeSqlNew(ver_id, service_code, product_code, feecode, "", djfsrid, "", dic.Get("RID").ToString());
                                                    if (sql != "")
                                                    {
                                                        DrawExcel(cells, ref rowIndex, sql, ref ifTitle, 0);
                                                    }
                                                    //List<string> sqll = getFeeSqlTry(ver_id, service_code, product_code, feecode, "", djfsrid, "");
                                                    //foreach (string sqls in sqll)
                                                    //{
                                                    //    DrawExcel(cells, ref rowIndex, sqls, ref ifTitle, 0);
                                                    //}
                                    ...
                                    foreach (DataRow dr in dtdjfswjc.Rows)
                                    {
                                        ...
                                        sql = getFeeSqlNew(ver_id, service_code, product_code, feecode, "", djfsrid2, "", dic.Get("RID").ToString());
                                        if (sql != "")
                                        {
                                            DrawExcel(cells, ref rowIndex, sql, ref ifTitle, 0);
                                        }
                                        //List<string> sqll = getFeeSqlTry(ver_id, service_code, product_code, feecode, "", djfsrid2, "");
                                        //foreach (string sqls in sqll)
                                        //{
                                        //    DrawExcel(cells, ref rowIndex, sqls, ref ifTitle, 0);
                                        //}
                                ...
                                {
                                    sql = getFeeSqlNew(ver_id, service_code, product_code, feecode, "", "", "", dic.Get("RID").ToString());
                                    if (sql != "")
                                    {
                                        DrawExcel(cells, ref rowIndex, sql, ref ifTitle, 0);
                                    }
                                    //List<string> sqll = getFeeSqlTry(ver_id, service_code, product_code, feecode, "", "", "");
                                    //foreach (string sqls in sqll)
                                    //{
                                    //    DrawExcel(cells, ref rowIndex, sqls, ref ifTitle, 0);
                                    //}
        ...
        private void CreateExcelEng(string main_id, string ver_id, out string fileName, out string filePath, out string msg, string type)
        {
            try
            {
                ...
                //                string sql_bporg = @"select t1.original,t1.bjname as ""报价名称"",t2.bpname as ""客户名称"",t2.bpcode as ""客户代码"",t3.orgname as ""报价组织"",t3.orgcode as ""组织代码"",to_char(t4.dtfrom,'yyyy/mm/dd') as ""起始日期"",to_char(t4.dtto,'yyyy/mm/dd') as ""截止日期"",t4.CONTRSCTNUM
                //from SQM_BJ_MAIN_BASIC t1 left join sqm_bj_bp t2 on t1.rid = t2.mrid left join sqm_bj_org t3 on t1.rid = t3.mrid left join sqm_bj_ver t4 on t1.rid = t4.mrid where t1.rid = '" + main_id + "'";
                string sql_bporg = @"select t1.original,t1.bjname_en as ""报价名称"",t2.bpname as ""客户名称"",t2.bpcode as ""客户代码"",t3.orgname as ""报价组织"",t3.orgcode as ""组织代码"",to_char(t4.dtfrom,'yyyy/mm/dd') as ""起始日期"",to_char(t4.dtto,'yyyy/mm/dd') as ""截止日期"",t4.CONTRSCTNUM
from SQM_BJ_MAIN_BASIC t1 left join sqm_bj_bp t2 on t1.rid = t2.mrid left join sqm_bj_org t3 on t1.rid = t3.mrid left join sqm_bj_ver t4 on t1.rid = t4.mrid where t1.rid = '" + main_id + "'";
                DataTable bjDt = DataHelper.QueryDataTable(sql_bporg);
                string bnname = "";
                string bpcode = "";//客户代码
                ...
                if (bjDt.Rows.Count > 0)
                {
                    ...
                    string bjzzsql = @"select * from mdm_org where ORGKEY='000000" + orgcode + "' and LANGTYPE='E'";
...
case when ""计费因子"" is not null then ""单价"" || '/' || to_char(""计费因子"") else ""单价"" end as ""单价"",""最低收费"",""费用说明"",""包干费"" from (" + sql_val_mb + ")";
        ...
        public string getFeeSqlEng(string ver_id, string service_code, string product_code, string fee_code, string unit, string djfsrid, string gdzrid)
        {
            ...
            string sql_val = "";
            // 获取数据源
            sql_val = getSourceSqlEng(fee_code, service_code, product_code, ver_id, unit, "mb", djfsrid, gdzrid);
            string bjfs = DataHelper.QueryValue("select bjfs from sqm_bj_psf where fee_code = '" + fee_code + "' and service_code = '" + service_code + "' and product_code = '" + product_code + "' and vrid = '" + ver_id + "'") + "";
            if (bjfs != "1" && bjfs != "2")// At cost 不处理计费基础
            {
                // 获取计费基础字段名,是否在模板中加入以及如何加入这些基础
...
case when ""计费因子"" is not null then ""单价"" || '/' || to_char(""计费因子"") else ""单价"" end as ""单价"",""最低收费"",""费用说明"",""包干费"",""服务代码"",""费目代码"" from (" + sql_val_mb + ")";
        ...
        public string getSourceSql(string feecode, string srvcode, string prdcode, string vrid, string unit, string type, string djfsrid, string gdzrid)
        {
            ...
            string feecalcid = "";
            string bjrid = "";
            string sql = "select distinct t1.rid as psfrid,t2.rid as bjrid from sqm_bj_psf t1 left join sqm_modebj_val t2 on t1.rid = t2.feecalcid where t1.fee_code = '" + feecode + "' and t1.service_code = '" + srvcode + "' and t1.product_code = '" + prdcode + "' and t1.vrid = '" + vrid + "'";
            DataTable dt = DataHelper.QueryDataTable(sql);
            if (dt.Rows.Count > 0)
            {
      ...
      --     when (t2.BJPRICE < t2.minprice or t2.bjprice > t2.maxprice) and IFUPDATE = '1' then to_char(case when --t2.CURRENCY = '{0}' or upper(t2.CURRENCY) = '{1}' then trim('.' from to_char(t2.BJPRICE,'fm999999990.99999'))- else -(to_char(nvl(t2.CURRENCY,'')) || trim('.' from to_char(t2.BJPRICE,'fm999999990.99999'))) end) || '(报价-超限1)'
      ...
      --end as ""报价"",
                       case
                         ...
                         when t1.BJFS = '2' then
                          '单票单询'
                         when (t2.BJPRICE < t2.minprice or
                              t2.bjprice > t2.maxprice) and IFUPDATE = '1' then
                          to_char((to_char(nvl(t2.CURRENCY, '')) ||
                                  trim('.' from to_char(t2.BJPRICE,
                                                'fm999999990.99999')))) ||
                          '(报价超限1)'
                         when t2.BJPRICE < t2.minprice or
                              t2.bjprice > t2.maxprice and
                              (IFUPDATE = '2' or IFUPDATE is null) then
                          to_char((to_char(nvl(t2.CURRENCY, '')) ||
                                  trim('.' from to_char(t2.BJPRICE,
                                                'fm999999990.99999')))) ||
                          '(报价超限2)'
...
left join (select bgfzrid,wm_concat(to_char(service_name) || '--' || to_char(fee_name)) as bgf from sqm_bj_psf where bgfzrid = '" + feecalcid + @"' group by bgfzrid,service_name,fee_name) t3 on t3.bgfzrid = m1.rid
order by to_number(regexp_substr(""报价"",'\d+(.\d+)?',1)) ", wherepur);//replace(wm_concat(to_char(calcname || ':' || memo)),',',chr(10)) as memo
        ...
        public string getSourceSqlEng(string feecode, string srvcode, string prdcode, string vrid, string unit, string type, string djfsrid, string gdzrid)
        {
            string currency_cn = "人民币";
            string currency_en = "CNY";
            // 获取计费基础字段名
  ...
  --    end as ""报价"",
        ...
        and t1.rid = '" + feecalcid + "' ";
                sql_val += wheredjfs;
                sql_val += @") m1 left join 
        (select distinct feeunit_en as feeunit,fsfysm_en as fsfysm,case when (gdzrid is not null or gdzrid <> '') then gdzrid else djfsrid end as djfsrid from sqm_fee_pur_ref) m2 on m1.djfsrid = m2.djfsrid ";
                sql_val += string.Format(@"left join (select case when gdzrid is not null or gdzrid <> '' then gdzrid else djfsrid end as djfsrid,
       replace(to_char(wm_concat(to_char(memo))),',',';') as memo
from sqm_fee_calc_ref where 1 = 1 and status = '1'
{0}
...
order by to_number(regexp_substr(""报价"",'\d+(.\d+)?',1)) ", wherepur);//replace(wm_concat(to_char(calcname || ':' || memo)),',',chr(10)) as memo
        ...
        //}
        public string getFeeSqlNew(string ver_id, string service_code, string product_code, string fee_code, string unit, string djfsrid, string gdzrid, string rid)
        {
            string sql_val_mb = "";
            string columns = "";
            string sql_val = "";
            // 获取数据源
            sql_val = getSourceSqlNew(fee_code, service_code, product_code, ver_id, unit, "mb", djfsrid, gdzrid, rid);
            string bjfs = DataHelper.QueryValue("select bjfs from sqm_bj_psf where fee_code = '" + fee_code + "' and service_code = '" + service_code + "' and product_code = '" + product_code + "' and vrid = '" + ver_id + "'") + "";
            if (bjfs != "1" && bjfs != "2")// At cost 不处理计费基础
            {
                // 获取计费基础字段名,是否在模板中加入以及如何加入这些基础
...
case when ""计费因子"" is not null then ""单价"" || '/' || to_char(""计费因子"") else ""单价"" end as ""单价"",""最低收费"",""费用说明"",""包干费"" from (" + sql_val_mb + ")";
        ...
        public string getFeeSqlEngNew(string ver_id, string service_code, string product_code, string fee_code, string unit, string djfsrid, string gdzrid,string rid)
        {
            string sql_val_mb = "";
            string columns = "";
            string sql_val = "";
...
case when ""计费因子"" is not null then ""单价"" || '/' || to_char(""计费因子"") else ""单价"" end as ""单价"",""最低收费"",""费用说明"",""包干费"",""服务代码"",""费目代码"" from (" + sql_val_mb + ")";
        ...
        public string getSourceSqlNew(string feecode, string srvcode, string prdcode, string vrid, string unit, string type, string djfsrid, string gdzrid, string rid)
        {
            string currency_cn = "人民币";
            string currency_en = "CNY";
            // 获取计费基础字段名
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["PSFRID"].ToString() == rid)
                    {
                        bjrid = dt.Rows[i]["BJRID"] + "";
                        feecalcid = dt.Rows[i]["PSFRID"] + "";
                    }
                }
            }
            string bjfs = DataHelper.QueryValue("select bjfs from sqm_bj_psf where fee_code = '" + feecode + "' and service_code = '" + srvcode + "' and product_code = '" + prdcode + "' and vrid = '" + vrid + "'") + "";
