<%@ Page Language="C#" MasterPageFile="~/Masters/Ext/Site.Master" AutoEventWireup="true"
    CodeBehind="MGrpSelect.aspx.cs" Inherits="Aim.Portal.Web.CommonPages.MGrpSelect" %>
...
<asp:Content ID="HeadContent" ContentPlaceHolderID="HeadHolder" runat="server">
    ...
    <script type="text/javascript">
        ...
        function onSelPgLoad() {
            ...
            SelSelViewUrl = "GrpSelView.aspx?CompanyId=" + $.getQueryString({ "ID": "CompanyId" }) + "&ParentId=" + $.getQueryString({ "ID": "ParentId" }) + "&showrole=" + $.getQueryString({ "ID": "showrole" });
        ...
        function InitSelections(pval) {
            ...
            if ($.getQueryString({ ID: "para" })) {
                var idEle = $.getQueryString({ ID: "para" }).split(":")[0] || "";
                var nameEle = $.getQueryString({ ID: "para" }).split(":")[1] || "";
                if (!window.opener) {
                    ids = ($("#" + idEle, window.dialogArguments.document).val() || "").split(",");
                    names = ($("#" + nameEle, window.dialogArguments.document).val() || "").split(",");
                }
            }
