        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (RequestActionString == "savechanges")
            {
                ...
                for (int i = 0; i < adds.Length; i++)
                {
                    ...
                    {
                        ROLEID = RoleId,
                        SIMPLENAME = addedName[i].Contains("-") ? addedName[i].Substring(addedName[i].IndexOf('-') + 1) : addedName[i],
                        ROLENAME = SysEnumeration.Find(RoleId).Name,
                        PERMISID = adds[i],
            ...
            {
                DataTable dtPer = DataHelper.QueryDataTable("SELECT ENUMERATIONID as \"id\",CODE,NAME as \"text\",ISLEAF as \"leaf\" FROM SYSENUMERATION WHERE NAME='按钮权限' ORDER BY sortindex");
