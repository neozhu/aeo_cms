    ...
    <script type="text/javascript">
        ...
        function setPgUI() {
            ...
            selCmdPanel = new Ext.ux.AimPanel({
                ...
                items: [{
                    xtype: 'button', text: '选择', handler: function() {
                        ...
                        else if (usrViewFrame.contentWindow.GetSelections) {
                            recs = usrViewFrame.contentWindow.GetSelections();
                        }
                ...
                cmdPanel = new Ext.ux.AimPanel({
                    ...
                    items: [{
                        ...
                        handler: function() {
                            ...
                            if (window.external) {
                                try {
                                    window.external.GetUsers(usrs.UserID + ";" + usrs.Name);
                                    return;
                                } catch (e) { }
                            }
                    ...
                    items: [{ xtype: 'box', region: 'north', applyTo: 'header', height: 30 },
            {
                region: 'west',
                split: true,
                hidden: $.getQueryString({ "ID": "hiddenLeft" }) == "true",
                margins: '0 0 0 0',
                cls: 'empty',
                width: 200,
            ...
            },
            {
                ...
                cls: 'empty',
                bodyStyle: 'background:#f1f1f1',
                html: '<iframe width="100%" height="100%" id="usrViewFrame" name="frameContent" frameborder="0" src="UsrSelView.aspx?DeptId=' + $.getQueryString({ "ID": "DeptId" }) + '"></iframe>'
            }, selPanel, cmdPanel]
                });
            }
            ...
            function InitSelections() {
                ...
                var unames = unescape((pval["Name"] || $.getQueryString({ ID: "Name", DefaultValue: "" }) || "")).split(",");
                if ($.getQueryString({ ID: "para" })) {
                    var userIDEle = $.getQueryString({ ID: "para" }).split(":")[0] || "";
                    var nameEle = $.getQueryString({ ID: "para" }).split(":")[1] || "";
                    if (!window.opener) {
                        uids = ($("#" + userIDEle, window.dialogArguments.document).val() || "").split(",");
                        unames = ($("#" + nameEle, window.dialogArguments.document).val() || "").split(",");
                    }
                }
