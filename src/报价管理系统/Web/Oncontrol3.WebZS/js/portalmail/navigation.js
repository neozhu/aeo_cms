...
; Neter.Navigation = function (options) {
    ...
    this.defaults = {
        ...
        closeButtonCssList: [
            ...
            'neter-navigation-item-close-button-selected'
        ].join(' ')
    };
    ...
    this.method = {
         ...
         * 获取导航项配置信息
         * @ignore
         * @prop string 属性或自定义属性
         * @val mixed 属性的值
         */
        getOptions: function (prop, val) {
            var items = _this.handler.items;

            for (var i = 0, len = items.length; i < len; ++i) {
                var ret = null;
                ret = items[i][prop] && items[i][prop] == val ? items[i] : null;
                if (ret) { return ret; }
            }
         ...
         * @val mixed 属性的值
         */
        getIndexByProp: function (prop, val) {
            var items = _this.handler.items;

            for (var i = 0, len = items.length; i < len; ++i) {
                var ret = -1;
                ret = items[i][prop] && items[i][prop] == val ? i : -1;
                if (~ret) { return ret; }
            }
        ...
        getActiveNav: function () {
            ...
            for (var i = 0, len = items.length; i < len; ++i) {
                var ret = null;
                ret = items[i].active === true ? items[i] : null;
                if (ret) { return ret; }
            }
            return null;
        },
...
; Neter.apply(Neter.Navigation.prototype, {
     ...
     * @function
     * @name Neter.Navigation.prototype.getIndex
     * @prop {String} prop 属性或自定义属性
     * @prop {mixed} val 属性或自定义属性的值
     * @return {Number} item对应的索引值，-1为未找到
     */
    getIndexByProp: function (prop, val) {
        return this.method.getIndexByProp(prop, val);
    },
    /**
     * 获取导航项
     * @function
     ...
     * @return {Number} item对应的索引值，-1为未找到
     */
    getNav: function (prop, val) {
        return this.method.getOptions(prop, val);
    },
    /**
     * 获取当前活动导航项
     * @function
     * @name Neter.Navigation.prototype.getIndex
     * @return {Number} 当前活动导航项,null为未找到
     */
    getActiveNav: function () {
        return this.method.getActiveNav();
    }
});
