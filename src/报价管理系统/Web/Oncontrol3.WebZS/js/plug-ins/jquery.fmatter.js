...
;(function($) {
    ...
    //jnp add 
    $.fn.fmatter.showlink = function(cellval, opts, dataRow) {
    var op = { baseLinkUrl: opts.baseLinkUrl, showAction: opts.showAction || "", addParam: opts.addParam || "", target: opts.target, idName: opts.idName, idIndex: opts.idIndex, linkstyle: opts.linkstyle },
		target = "";
        if (!isUndefined(opts.colModel.formatoptions)) {
            op = $.extend({}, op, opts.colModel.formatoptions);
        }
        if (op.target) { target = 'target=' + op.target; }
        var idUrl = op.baseLinkUrl.indexOf('?') > 0 ? op.baseLinkUrl+'&' : op.baseLinkUrl + '?';
        idUrl += op.idName + '=' + dataRow[op.idIndex] + op.addParam;
        if (isString(cellval)) {	//add this one even if its blank string
            return "<a href='#' onclick=\"OpenWin('" + idUrl + "','" + op.target + "',CenterWin('" + op.linkstyle + "'))\">" + cellval + "</a>";
        } else {
            return $.fn.fmatter.defaultFormat(cellval, opts);
        }
    };
