    ...
    public class SQM_FMBJController : BaseController
    {
        ...
        public ActionResult SelectFMBJ()
        {
            ...
            try
            {
                ...
                string sql_val = "";
                sql_val = SearchBJSql(djrid, bjrid, feecode, ifbj, djfsrid, gdzrid, gdzkey, searchStr);
        ...
        public string SearchBJSql(string djrid, string bjrid, string feecode, string ifbj, string djfsrid, string gdzrid, string gdzkey, string searchStr)
        {
            ...
            if (!String.IsNullOrEmpty(ifbj))
            {
                ...
                {
                    //0 非报价项目，查询定价值表
                    sql_zval = string.Format(sql_val + ",A.MIN,A.MIN as DJMIN,'DJ' as DATASOURCE from SQM_MODEDJ_VAL A where A.DJSTATUS = '1' and A.FEECALCID='{1}' {0}" + searchStr + " union " + sql_val + ",A.MINBJPRICE as MIN,B.MIN as DJMIN,'BJ' as DATASOURCE from SQM_MODEBJ_VAL A left join SQM_MODEDJ_VAL B on A.DJRID=B.RID where A.FEECALCID='{2}' {0}", sql_ifbj, djrid, bjrid);
                }
                else
                {
                    sql_zval = string.Format(sql_val + ",A.MIN,A.MIN as DJMIN,'DJ' as DATASOURCE from SQM_MODEDJ_VAL A where A.DJSTATUS = '1' and A.FEECALCID='{1}' {0}" + " union " + sql_val + ",A.MINBJPRICE as MIN,B.MIN as DJMIN,'BJ' as DATASOURCE from SQM_MODEBJ_VAL A left join SQM_MODEDJ_VAL B on A.DJRID=B.RID where A.FEECALCID='{2}' {0}" + searchStr, sql_ifbj, djrid, bjrid);
                }
                return sql_zval;
            }
            sql_zval = string.Format(sql_val + ",A.MIN,A.MIN as DJMIN,'DJ' as DATASOURCE from SQM_MODEDJ_VAL A where A.DJSTATUS = '1' and A.FEECALCID='{1}' {0}" + " union " + sql_val + ",A.MINBJPRICE as MIN,B.MIN as DJMIN,'BJ' as DATASOURCE from SQM_MODEBJ_VAL A left join SQM_MODEDJ_VAL B on A.DJRID=B.RID where A.FEECALCID='{2}' {0}", sql_ifbj, djrid, bjrid);
                    ...
                    if (djval != "[]")
                    {
                        ...
                        if (!string.IsNullOrEmpty(idguidprice))
                        {
                            ...
                            for (int i = dtDj.Rows.Count - 1; i >= 0; i--)
                            {
                                ...
                                decimal hl = dt_Tcurrr.Rows.Count > 0 ? decimal.Parse(dt_Tcurrr.Rows[0]["UKURS"].ToString()) : 1;
                                ...
                                else
                                {
                                    ...
                                    if (oldsmvobj == null)
                                    {
                                        ...
                                        srcobj.BJPRICE = Convert.ToDecimal(bjpriceval);
        ...
        public string CODETONAME(string type, string code)
        {
            ...
            switch (type)
            {
                ...
                case "srv":
                    ...
                    break;
                case "fee":
                    var feelist = DataHelper.QueryDictList("SELECT MDM_FEE.TCET084,MDM_FEE.TEXTDESC FROM MDM_FEE");
