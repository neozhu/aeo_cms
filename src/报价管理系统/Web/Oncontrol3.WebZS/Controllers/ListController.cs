...
namespace Oncontrol3.Web.Controllers
{
    ...
    public class ListController : Controller
    {
        ...
        public ActionResult SSOXSY()
        {
            ...
            string Token = Request.QueryString["access_token"];//Token
            ...
            if (isSuccess)
            {
                ...
                if (CustomerModel != null && CustomerModel.Count > 0)
                {
                    ...
                    cusmodel.Inner = CustomerModel[0].internalCompany__c;
                }
                else
                {
                ...
                {
                    string prdcode = itm.productName;//接口修改字段
                    string diversion = itm.customItem128__c;//接口修改字段
                    if (diversion.Contains("运输"))
                    {
                        diversion = "运输";
                    }
                    if (diversion.Contains("海运"))
                    {
                        diversion = "海运";
                    }
                    if (diversion.Contains("空运"))
                    {
                        diversion = "空运";
                    }
                    if (diversion.Contains("供应链"))
                    {
                        diversion = "供应链";
                    }
                    ...
                    //    cp.PRODUCTSCODE = "";
                    //    cp.PRODUCTSNAME = prdcode;//
                    //    cp.DIVISION = diversion;
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SSOXSY_FOREDIT()
        {
            string UserId = Request.QueryString["UserId"];//用户ID
            string Token = Request.QueryString["access_token"];//Token
            string url = System.Configuration.ConfigurationManager.AppSettings["XSYUSERINFOURL"];
            string keyValue = Request.QueryString["keyValue"];//用户ID
            string zversion = Request.QueryString["zversion"];//用户ID
                                                              //bool isSuccess = VilToke(url, Token);
                                                              //if (isSuccess)
                                                              //{
            ...
            //bl.CREATEUSER = UserId;
            //bl.CREATETIME = DateTime.Now;
            //bl.DateSpan = DateTime.Now.ToString("yyyyMMddhhmmssfff");
            ////bl.HASH = Token;//超过长度
            //bl.DoCreate();
            ...
            GetDataFromXSYByCustomerNo.pbCreateGetCRM[] bodyList = new GetDataFromXSYByCustomerNo.pbCreateGetCRM[1];
            body.customerno = cusobj.BPCODE;
            body.businessno = busobj.BIZID;
            bodyList[0] = body;
            msgResponse content = data.CallCreateGetCRM(head, bodyList);
            msgResponse[] list = content.list;
            JsonParser jo = JsonConvert.DeserializeObject<JsonParser>(list[0].originMessage);
            ...
            contrsctmodel.contrsctnum = Contrsc != null && Contrsc.Count > 0 ? Contrsc[0].customItem153__c : "";
            contrsctmodel.contrsctname = Contrsc != null && Contrsc.Count > 0 ? Contrsc[0].customItem155__c : "";
            SQM_BJ_VER[] versrcobj = SQM_BJ_VER.FindAllByProperties(SQM_BJ_VER.Prop_MRID, keyValue);//, SQM_BJ_VER.Prop_ZVER

            foreach (var item in versrcobj)
            {
                if (string.IsNullOrEmpty(item.CONTRSCTNUM))
                {
                    item.CONTRSCTNUM = contrsctmodel.contrsctnum;
                    item.DoUpdate();
                }
            }

            #endregion
            return Redirect("/QM_Price_N/QM_PriceEdit?keyValue=" + keyValue + "&SystemFrom=XSY&zversion=" + zversion);
            //}
            //else
            //{
            //    return Content("验证不通过,请联系管理员");
    ...
    public class Customer
    {
        ...
        //是否内部 0 否  1 是
        public string internalCompany__c { get; set; }
    }
    /// <summary>
    /// 商机Model
    /// </summary>
