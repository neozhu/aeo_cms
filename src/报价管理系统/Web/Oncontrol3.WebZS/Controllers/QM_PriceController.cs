...
namespace Oncontrol3.Web.Controllers
{
    ...
    public class QM_PriceController : BaseController
    {
        ...
        public ActionResult RATEFWATM(string keyvalue, string zver, string tjzlcs, string qqq)
        {
            ...
            {
                removeFailedFWA(fwafagrmntid044List, keyvalue, zver);
                string path = System.AppDomain.CurrentDomain.BaseDirectory + "MDMBPLOG";
                if (!Directory.Exists(path))
                {
                    Directory.CreateDirectory(path);
                }
                string name = path + "/" + "MDM_BP" + DateTime.Now.ToString("yyyyMMdd") + ".html";
                FileInfo file = new FileInfo(name);
                if (!file.Exists)
                {
                    StreamWriter sw = file.CreateText();
                    sw.WriteLine("启动时间：" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                    sw.WriteLine(ex);
                    sw.Close();
                }
                else
...
SELECT * FROM ( 
       ...
       ORDER BY CREATETIME DESC
        ...
        {
            string sql = string.Format("select rid,zver,status,dtfrom,dtto,createuser,modifytime,memo,workflow,fbreasoncode,fbreasonother,fbmemo from sqm_bj_ver where mrid = '{0}' order by createtime desc", keyvalue);
            var data = DataHelper.QueryDictList(sql);
            string fwa = "";
            foreach (var item in data)
            {
        ...
        public void SetMemo(string vrid, string memo)
        {
            ...
            if (!string.IsNullOrEmpty(mainobj.XSYBJID))
            {
                ...
                _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + mrid + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;
                ubody[0] = _ubody;
                _ubodys.data = ubody;
                ubodys[0] = _ubodys;
                uwb.CallUpdateQuotation(uhead, ubodys);
