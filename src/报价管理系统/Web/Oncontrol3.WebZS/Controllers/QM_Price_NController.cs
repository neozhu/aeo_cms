...
namespace Oncontrol3.Web.Controllers
{
    ...
    public class QM_Price_NController : BaseController
    {
        public ActionResult getBpcode9(string customerid, string mrid, string busArray)
        {
            string sql = "";
            string gylnum = "0";
            DataTable bpdt = null;
            try
            {
                ...
                if (gylnum == "0")
                {
                    ...
                    //    }
                    //}
                    string countnum = DataHelper.QueryValue(string.Format(@"select * from sqm_bj_psf t 
                                        join SQM_CRM_PRODUCT_REF t2
                                        on t.product_code = t2.product
                                        where t.mrid = '{0}'
                                        and t2.businessunit = '供应链'
                                        ", mrid)) + "";
                    if (countnum != "0")
                    {
                        gylnum = "1";
                ...
                {
                    bpdt = DataHelper.QueryDataTable("select BPKEY from MDM_BP where BPKEY like '" + customerid + "%' and length(BPKEY) = 9");
                }
            }
            catch (Exception)
            {
        ...
        /// <returns></returns>
        public ActionResult SaveSJBJ(string keyvalue, string zversion, string dtfrom, string dtto, string contrsctnum, string memo, string bpcode9, string WAERS)
        {
            var flag = true;
            var rtnmessga = "保存成功";
            string code = "1";
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(mainobj.XSYBJID))
                {
                    ...
                    _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + keyvalue + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;//报价地址
                    ubody[0] = _ubody;
                    _ubodys.data = ubody;
                    ubodys[0] = _ubodys;
        ...
        public ActionResult RATEFWATM(string keyvalue, string zver, string tjzlcs, string qqq)
        {
            List<string> fwafagrmntid044List = new List<string>();
            List<List<string>> lii = JsonConvert.DeserializeObject<List<List<string>>>(qqq);
            bool containjsjc = false;
            for (int i = 0; i < lii.Count; i++)
            {
                ...
                string fmrid = "[\"" + lii[i][0] + "\",\"" + lii[i][1] + "\"]";
                string alonefee = lii[i][4];
                var dtt = FMBJ(fmrid, alonefee);
                if (dtt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtt.Rows)
                    {
                        string lfdsql = string.Format(@"select * from LFDCONFIG where JSJC='{0}'", dr["CALCCODE"].ToString());
                        var dtable = DataHelper.QueryDataTable(lfdsql);
            ...
            A2SLabel:
                ...
                //psf表信息
                sql = string.Format("SELECT * FROM SQM_BJ_PSF WHERE VRID = '{0}' AND CHOOSESTATUS = '1' AND ( BJSTATAUS = '2' OR BJSTATAUS = '5' OR BJSTATAUS = '7' ) ORDER BY PRODUCT_CODE, SERVICE_CODE, FEE_CODE ", vrid);
                //过滤掉at cost且被包干的费目
                ...
                string sybsql = string.Format("select distinct substr(PRODUCT_CODE, 0, 1) syb from  ( SELECT PRODUCT_CODE FROM SQM_BJ_PSF WHERE VRID = '{0}' AND CHOOSESTATUS = '1' AND ( BJSTATAUS = '2' OR BJSTATAUS = '5' or BJSTATAUS = '7' )  ORDER BY PRODUCT_CODE, SERVICE_CODE, FEE_CODE )", vrid);
                ...
                foreach (string strSyb in sybproductcodeslist)
                {
                    ...
                    fwa.DIM_WT_PROFILE = tjzlcs;//dtBJMB.Rows[0]["MVFILECODE"] + "";//体积重量参数文件  报价台头取
                    ...
                    fwa.DOC_CURRENCY = string.IsNullOrEmpty(dtBJMB.Rows[0]["BJTCURR"] + "") ? "CNY" : dtBJMB.Rows[0]["BJTCURR"] + "";//"CNY";//凭证货币    报价货币
                    ...
                    foreach (DataRow drbp in dtBP.Rows)
                    {
                        ...
                        #region 之前判断是否内部公司逻辑  来自crm, 现在改成来自于接口
                        //if (!bNBGS)
                        //{
                        ...
                        bpsql = string.Format(" select * from mdm_bp where BPKEY like '%{0}%' ", sdrbp);
                        ...
                        party.UUID001 = DBKeyHelper.HexToBytes(bpsql);
                        partylist.Add(party);
                    }
                    ...
                    if (!string.IsNullOrEmpty(strFWA703))//先判断报价是否生成过协议，是则先获取已有协议，进行修改
                    {
                        try
                        {
                            ...
                            foreach (string product_code in productcodeslist)
                            {
                                ...
                                //给空海运服务类型打勾
                                if (fwa.FAGTYPEID103 == "Z301" || fwa.FAGTYPEID103 == "Z201")
                                {
                                    Hashtable ht = new Hashtable();
                                    foreach (DataRow dr in dtpsf.Select("PRODUCT_CODE = '" + product_code + "'"))
                                    {
                                        Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEM fag_item702 = new Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEM();
                                        var srvcode1 = dr["SERVICE_CODE"] + "";
                                        fag_item702.SERVICE_TYPE = srvcode1;//服务类型   报价的服务代码
                                        fag_item702.ZBJFW = "X";
                                        fag_item702.PAR_KEY = strHex32;
                                        if (ht.Contains(srvcode1))
                                        {
                                            continue;
                                        }
                                        ht.Add(srvcode1, srvcode1);
                                        fag_itemlist702.Add(fag_item702);
                                    }
                                }
                                    ...
                                    if ((drpsf["BJFS"] + "" != "1") && null != feecalcntdt && feecalcntdt.Rows.Count > 0)
                                    {
                                        for (int i = 0; i < feecalcntdt.Rows.Count; i++)
                                        {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        if (bMIN)
                                                        {
                                                            ...
                                                            if (bjvalsdt.Rows.Count == 1 && calccodestrc.Count == 1)
                                                            {
                                                                ...
                                                                if (!string.IsNullOrEmpty(tjzlcs) && containjsjc)
                                                                {
                                                                    item_calcrule.QTY_VALUE = "1000";
                                                                }
                                                                ...
                                                                {
                                                                    List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT> ITEM_TEXT_LI = new List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT>();
                                                                    Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT itte = new Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT();
                                                                    itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签2
                                                                    ITEM_TEXT_LI.Add(itte);
                                                                    tccs_item.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                                }
                                                                tccs_itemlist.Add(tccs_item);
                                            ...
                                            else
                                            {
                                                ...
                                                if (tccs_itemeval.TCCALCRESINS040 == "STND")
                                                {
                                                    List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT> ITEM_TEXT_LI = new List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT>();
                                                    Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT itte = new Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT();
                                                    itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签2
                                                    ITEM_TEXT_LI.Add(itte);
                                                    tccs_itemeval.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                }
                                                ...
                                                foreach (string strgdzid in gdzidlist)
                                                {
                                                    ...
                                                    if (tccs_itemsum.TCCALCRESINS040 == "STND")
                                                    {
                                                        List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT> ITEM_TEXT_LI = new List<Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT>();
                                                        Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT itte = new Z2FM_SQ_FWA_MODIFYIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT();
                                                        itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签2
                                                        ITEM_TEXT_LI.Add(itte);
                                                        tccs_itemsum.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                    }
                                                            ...
                                                            if (bMIN)
                                                            {
                                                                if (bjvalsdt.Rows.Count == 1 && calccodestrc.Count == 1)
                                                                {
                                                                    ...
                                                                    if (tccs_itemgdz.TCCALCRESINS040 == "STND")
                                                                    {
                                                                        ...
                                                                        itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签2
                                                                        ITEM_TEXT_LI.Add(itte);
                                                                        tccs_itemgdz.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                                    }
                                                                    tccs_itemlist.Add(tccs_itemgdz);
                                                                ...
                                                                foreach (string calcref in calccodestrcref)
                                                                {
                                                                    ...
                                                                    if (isex)
                                                                    {
                                                                        ...
                                                                        if (tccs_itemref.TCCALCRESINS040 == "STND")
                                                                        {
                                                                            ...
                                                                            itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签2
                                                                            ITEM_TEXT_LI.Add(itte);
                                                                            tccs_itemref.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                                        }
                                                                        tccs_itemlist.Add(tccs_itemref);
                            ...
                            if (!string.IsNullOrEmpty(mainobj.XSYBJID))
                            {
                                ...
                                _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + keyvalue + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zver;//报价地址
                                ubody[0] = _ubody;
                                _ubodys.data = ubody;
                                ubodys[0] = _ubodys;
                                uwb.CallUpdateQuotation(uhead, ubodys);
                    ...
                    foreach (string product_code in productcodeslist)
                    {
                        ...
                        if (fwa.FAGTYPEID103 == "Z301" || fwa.FAGTYPEID103 == "Z201")
                        {
                            ...
                            foreach (DataRow dr in dtpsf.Select("PRODUCT_CODE = '" + product_code + "'"))
                            {
                                Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEM fag_item701 = new Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEM();
                                var srvcode1 = dr["SERVICE_CODE"] + "";
                                fag_item701.SERVICE_TYPE = srvcode1;//服务类型   报价的服务代码
                                fag_item701.ZBJFW = "X";
                                fag_item701.PAR_KEY = strHex32;
                                if (ht.Contains(srvcode1))
                                {
                                    continue;
                                }
                                ht.Add(srvcode1, srvcode1);
                                fag_itemlist.Add(fag_item701);
                            }
                        }
                                            ...
                                            else
                                            {
                                                ...
                                                if (bMIN)
                                                {
                                                    if (bjvalsdt.Rows.Count == 1 && calccodestrc.Count == 1)
                                                    {
                                                        ...
                                                        if (tccs_item.TCCALCRESINS040 == "STND")
                                                        {
                                                            List<Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT> ITEM_TEXT_LI = new List<Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT>();
                                                            Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT itte = new Z2FM_SQ_FWA_CREATEIT_FWAFAG_ITEMTCCS_ROOTTCCS_ITEMITEM_TEXT();
                                                            itte.DESCRIPTION = drpsf["OTHER_NAME"].ToString();//标签1
                                                            ITEM_TEXT_LI.Add(itte);
                                                            tccs_item.ITEM_TEXT = ITEM_TEXT_LI.ToArray();
                                                        }
                                                        tccs_itemlist.Add(tccs_item);
            ...
            rtnLabel:
                ...
                if (rtnflag && rtnfwa.Length > 0)
                {
                    ...
                    var mainobj1 = SQM_BJ_MAIN_BASIC.TryFind(keyvalue);//报价主数据信息//不创建商机
            ...
            {
                removeFailedFWA(fwafagrmntid044List, keyvalue, zver);
                return Content(new JsonMessage { Success = false, Data = null, Code = "-1", Message = ex.ToString() }.ToString());
            }
        }
        ...
        public ActionResult QM_PriceEdit()
        {
            ...
            //报价类型
            //string bjtype = DataHelper.QueryValue(string.Format(@"select FBPRICE from SQM_BJ_MAIN_BASIC where RID='{0}'", Request["keyValue"].ToString())) + "";
            var mainobj = SQM_BJ_MAIN_BASIC.TryFind(Request["keyValue"].ToString());
            ViewBag.bjtype = mainobj.FBPRICE;
            ViewBag.Start = mainobj.DTFROM;
            ViewBag.end = mainobj.DTTO;
...
SELECT * FROM ( 
       ...
       ORDER BY CREATETIME DESC
        ...
        public ActionResult SendEmail(string strmailto, string copy, string title, string body, string RID, string vrid)
        {
            ...
            for (var i = 0; i < Strmailtos.Length; i++)
            {
                try
                {
                    ...
                    var mainobj = SQM_BJ_MAIN_BASIC.TryFind(sbv.MRID);
                    //不创建商机
                    if (!string.IsNullOrEmpty(mainobj.XSYBJID))
                    {
                        #region  作废 报价接口 
                        ...
                        _ubody.id = mainobj.XSYBJID;
                        _ubody.customItem3__c = zver;//报价版本 //测试
                        _ubody.customItem4__c = InterfaceFormat.FormatStatusXSY("4", "Z");//报价状态
                        _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + sbv.MRID + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zver;//报价地址
                        ubody[0] = _ubody;
                        _ubodys.data = ubody;
                        ubodys[0] = _ubodys;
                        uwb.CallUpdateQuotation(uhead, ubodys);
                    ...
                    else
                    {
                        ...
                        BJWriteback.pbCreateQuotation _bodys = new BJWriteback.pbCreateQuotation();
                        BJWriteback.pbCreateQuotation[] bodys = new BJWriteback.pbCreateQuotation[1];
                        BJWriteback.pbCreateQuotationData[] bodyd = new BJWriteback.pbCreateQuotationData[1];
                        BJWriteback.pbCreateQuotationData _body = new BJWriteback.pbCreateQuotationData();

                        SQM_BJ_ORG orgobj = SQM_BJ_ORG.FindFirstByProperties(SQM_BJ_ORG.Prop_MRID, sbv.MRID);
                        SQM_BJ_BIZ busobj = SQM_BJ_BIZ.FindFirstByProperties(SQM_BJ_BIZ.Prop_MRID, sbv.MRID);
                        SQM_BJ_BP cusobj = SQM_BJ_BP.FindFirstByProperties(SQM_BJ_BP.Prop_MRID, sbv.MRID);
                        //_body.customItem1__c = mainobj.BJNAME;//报价编号  不填
                        _body.lockStatus = "2";
                        //_body.quotationTitle = "";报价名称
                        _body.customItem3__c = zver;//报价版本 
                        _body.customItem4__c = InterfaceFormat.FormatStatusXSY("4", "Z");//报价状态
                        _body.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + sbv.MRID + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zver;//报价地址
                        _body.customItem6__c = orgobj.ORGCODE;//mainobj.BJNAME;//操作组织 
                        _body.customItem7__c = sbv.MRID;//mainobj.BJNAME;//报价ID
                        _body.ownerId = Oncontrol3.Web.Helpers.SQMHelper.getStaffKey(); //mainobj.BJNAME;//所有人
                        _body.entityType = "855577209438913";//业务类型写死
                        _body.quotationEntityRelAccount = cusobj.BPCODE;//客户编码
                        _body.quoteTime = DateToTicks(DateTime.Now).ToString();//报价时间
        ...
        public ActionResult GetFbPrice(string bjname)
        {
            //where createuser = '{1}'
            var user = Oncontrol3.Web.Helpers.SQMHelper.getStaffKey();
            string sql = string.Format("select a.*,b.zver,b.modifytime as mdtime,b.status as sta,c.bpname from sqm_bj_main_basic a left join( select v.*, to_number(replace(v.zver, 'V', '')) from sqm_bj_ver v inner join( select max(to_number(replace(zver, 'V', ''))) vs, mrid from sqm_bj_ver group by mrid) a on v.mrid = a.mrid and to_number(replace(v.zver, 'V', '')) = a.vs) b on a.rid = b.mrid left join sqm_bj_bp c on a.rid = c.mrid where a.fbprice='1' and a.affiliation = '{1}'  and a.bjname like '%{0}%'  order by a.createtime desc", bjname.Trim(), user);
            var data = DataHelper.QueryDictList(sql);
            return Content(JsonHelper.GetJsonString(data));
        }
        /// <summary>
        ...
        public ActionResult GetAllByRid(string keyvalue, string ver)
        {
            ...
            //foreach (var fee in feeArray)
            //{
            //    //国际段显示自定义报价 Rank 20191205
            //    if (fee[0].ToString() == "OO05" && fee[2].ToString() == "O00008")
            //    {
            //        flag=true;
            //    }
            //    else
        ...
        {
            string sql = string.Format("select rid,zver,status,to_char(dtfrom,'yyyy/MM/dd') dtfrom_T,to_char(dtto,'yyyy/MM/dd')  dtto_T,dtfrom,dtto,createuser,modifytime,memo,workflow,fbreasoncode,fbreasonother,fbmemo from sqm_bj_ver where mrid = '{0}' order by createtime desc", keyvalue);
            var data = DataHelper.QueryDictList(sql);
            string fwa = "";
            foreach (var item in data)
            {
        ...
        public ActionResult showHead(string keyvalue)
        {
            ...
            string sql = string.Format(@"SELECT sqm_bj_main_basic.bjname,
                                           sqm_bj_main_basic.bjtcurr,
                                           sqm_bj_biz.bizname,
                                           sqm_bj_biz.bizid,
                                           sqm_bj_biz.bizid,
                                           sqm_bj_bp.bpname,
                                      ...
                                      LEFT JOIN sqm_bj_biz
                                        ON sqm_bj_main_basic.rid = sqm_bj_biz.mrid
                                      LEFT JOIN sqm_bj_bp
                                        ON sqm_bj_main_basic.rid = sqm_bj_bp.mrid
                                      left join SQM_BJ_ORG
                                        on sqm_bj_main_basic.rid=sqm_bj_org.mrid
                                        where sqm_bj_main_basic.rid='{0}'", keyvalue);
            var data = DataHelper.QueryDictList(sql);
            return Content(JsonHelper.GetJsonString(data));
        }
        /// <summary>
        ...
        public ActionResult showFileeng(string RID)
        {
            string vrid = Request["vrid"] + "";
            string sql = string.Format("select uploadname,showmode,to_char(uploadtime,'yyyy-mm-dd hh24:mi:ss') as uploadtime,uploadurl from sqm_bj_ver_eng where mrid = '{0}' and rid = '{1}'", RID, vrid);
            var data = DataHelper.QueryDictList(sql);
            return Content(JsonHelper.GetJsonString(data));
        }
        /// <summary>
                    ...
                    else
                    {
                        foreach (var s in p.srvcodes)
                        {
                            foreach (var f in s.feecodes)
                            {
                                ...
                                //    srcobj.SERVICE_CODE = s.srvcode;
                                //    srcobj.SERVICE_NAME = CODETONAME("srv", s.srvcode);
                                //    srcobj.FEE_CODE = "CUSTOMIZE";//自定义
                                //    srcobj.FEE_NAME = "自定义报价";//CODETONAME("fee", f);
                                //    srcobj.BJSTATAUS = "0";
                                //    srcobj.STATUS = "1";
                                //    if (aloenfee == "1")
                                //    {
                            ...
                            else// 从报价值表拿数据
                            {
                                ...
                                else if (bjval.Rows.Count > 0)
                                {
                                    if (original == "")
                                    {
                                        ...
                                        //for (int i = 0; i < bjval.Rows.Count - 1; i++)
                                        //{
                                        //    var startdate = DateTime.Parse(bjval.Rows[i]["STARTDATE"].ToString());
                                        //    var startdate2 = DateTime.Parse(bjval.Rows[i + 1]["STARTDATE"].ToString());
                                        //    var enddate = DateTime.Parse(bjval.Rows[i]["ENDDATE"].ToString());
                                        //    var enddate2 = DateTime.Parse(bjval.Rows[i + 1]["ENDDATE"].ToString());
                                    ...
                                    string sqllll = "begin update sqm_bj_psf set bjstataus = '" + psfstatus + "' where rid = '" + bjrid + "';update sqm_modebj_val set ifbjitem = '1' where feecalcid = '" + bjrid + "'; end;";
        ...
        public void SetMemo(string vrid, string memo)
        {
            ...
            if (!string.IsNullOrEmpty(mainobj.XSYBJID))
            {
                ...
                _ubody.id = mainobj.XSYBJID;
                _ubody.customItem3__c = zversion;//报价版本 //测试
                _ubody.customItem4__c = InterfaceFormat.FormatStatusXSY("1", "Z");//报价状态
                _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + mrid + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;//报价地址
                ubody[0] = _ubody;
                _ubodys.data = ubody;
                ubodys[0] = _ubodys;
                uwb.CallUpdateQuotation(uhead, ubodys);
            ...
            else
            {
                ...
                //_body.quotationTitle = "";报价名称
                _body.customItem3__c = zversion;//报价版本 
                _body.customItem4__c = InterfaceFormat.FormatStatusXSY("1", "Z");//报价状态
                _body.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + mrid + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;//报价地址
                _body.customItem6__c = orgobj.ORGCODE;//mainobj.BJNAME;//操作组织 
        ...
        public ActionResult deleteVer(string vrid)
        {
            ...
            if (!string.IsNullOrEmpty(mainobj.XSYBJID))
            {
                ...
                _ubody.id = mainobj.XSYBJID;
                _ubody.customItem3__c = zversion;//报价版本 //测试
                _ubody.customItem4__c = InterfaceFormat.FormatStatusXSY("6", "Z");//报价状态
                _ubody.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + mrid + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;//报价地址
                ubody[0] = _ubody;
                _ubodys.data = ubody;
                ubodys[0] = _ubodys;
            ...
            else
            {
                ...
                //_body.quotationTitle = "";报价名称
                _body.customItem3__c = zversion;//报价版本 
                _body.customItem4__c = InterfaceFormat.FormatStatusXSY("6", "Z");//报价状态
                _body.customItem5__c = System.Configuration.ConfigurationManager.AppSettings["XSY_BACK_URL"] + "List/SSOXSY_FOREDIT?keyValue=" + mrid + "&UserId=" + Oncontrol3.Web.Helpers.SQMHelper.getStaffKey() + "&zversion=" + zversion;//报价地址
                _body.customItem6__c = orgobj.ORGCODE;//mainobj.BJNAME;//操作组织 
                _body.customItem7__c = mrid;//mainobj.BJNAME;//报价ID
                _body.ownerId = Oncontrol3.Web.Helpers.SQMHelper.getStaffKey(); //mainobj.BJNAME;//所有人
        ...
        public ActionResult saveNewVer(string mrid, string verdtto, string verdtfrom, string vermemo, string selectedArray, string zver, string pvrid)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(verdtfrom))
                {
                    ...
                    mainobj.DTFROM = verdtfroms;
                }
                if (!string.IsNullOrEmpty(verdtto))
                {
                    versrcobj.DTTO = DateTime.Parse(verdtto);
                    mainobj.DTTO = DateTime.Parse(verdtto);
                }
                versrcobj.MEMO = vermemo;
                versrcobj.DoCreate();
                mainobj.CREATETIME = DateTime.Now;
                mainobj.MODIFYTIME = DateTime.Now;
                mainobj.DoUpdate();
        ...
        public ActionResult GetPriceFromBp(string bjname)
        {
            ...
            string sql = string.Format("select a.*,b.zver,b.modifytime as mdtime,b.status as sta,c.bpname from sqm_bj_main_basic a left join( select v.*, to_number(replace(v.zver, 'V', '')) from sqm_bj_ver v inner join( select max(to_number(replace(zver, 'V', ''))) vs, mrid from sqm_bj_ver  group by mrid) a on v.mrid = a.mrid and to_number(replace(v.zver, 'V', '')) = a.vs) b on a.rid = b.mrid left join sqm_bj_bp c on a.rid = c.mrid where (a.fbprice<>'1'or a.fbprice is null) and a.affiliation = '{1}'  and a.bjname like '%{0}%' and c.bpcode = '{2}' order by a.createtime desc", bjname, user, bpcode);
            var data = DataHelper.QueryDictList(sql);
            string fwa = "";
            foreach (var item in data)
            {
        ...
        public ActionResult GetFBPriceFromBp(string bjname)
        {
            ...
            var user = Oncontrol3.Web.Helpers.SQMHelper.getStaffKey();
            string bpcode = Request["bpcode"];
            string sql = string.Format("select a.*,b.zver,b.modifytime as mdtime,b.status as sta,c.bpname from sqm_bj_main_basic a left join( select v.*, to_number(replace(v.zver, 'V', '')) from sqm_bj_ver v inner join( select max(to_number(replace(zver, 'V', ''))) vs, mrid from sqm_bj_ver group by mrid) a on v.mrid = a.mrid and to_number(replace(v.zver, 'V', '')) = a.vs) b on a.rid = b.mrid left join sqm_bj_bp c on a.rid = c.mrid where a.fbprice='1'and a.affiliation = '{1}'  and a.bjname like '%{0}%' and c.bpcode = '{2}' order by a.createtime desc", bjname, user, bpcode);
            var data = DataHelper.QueryDictList(sql);
            return Content(JsonHelper.GetJsonString(data));
        }
        ...
        public ActionResult CGSCWorkFlowXML(string rid, string vrid)//提交审批
        {
            ...
            else
            {
                ...
                string BJ = "0";//判断报价区间 0.报价区间内 1.报价区间外 2.非标报价 3.无定价报价
                ...
                if (FBSTATUS == true)
                {
                    try
                    {
                        ...
                        if (FlowKey == "0")
                        {
                            FlowID = "2561";
                            FlowName = "单事业部产品报价审批";
                            CPFZR = feilioahelper.GetCPFZR(vrid, rid) + "";
                        }
                        else if (FlowKey == "1")
                        {
                            FlowID = "2562";
                            FlowName = "跨事业部产品报价审批";
                            DCPFZR = feilioahelper.GetDCPFZR(vrid, rid) + "";
                        }
                        else if (FlowKey == "2")
                        {
                            FlowID = "2563";
...
<weaver.workflow.webservices.WorkflowRequestTableField>
        ...
        <weaver.workflow.webservices.WorkflowRequestTableField>
            ...
            <isMand>false</isMand>
                        ...
                        string XSY_XF_OA = System.Configuration.ConfigurationManager.AppSettings["XSY_XF_OA_port"];
                        if (dataTable != null && dataTable.Rows.Count > 0)
                        {
                            foreach (DataRow row in dataTable.Rows)
                            {
                                ...
                                {
                                    //FJ += "http://" + Request.Url.Host + ":" + XSY_XF_OA + "/Excel/output/" + UPLOADNAME + "|";
                                    FJ += XSY_XF_OA + "/Excel/output/" + UPLOADNAME + "|";
                        ...
                        xml = xml.Replace("[附件名称]", feilioahelper.GetFormatHTNAME(FjName.TrimEnd('|')));
                        ...
                        xml = xml.Replace("[产品负责人]", CPFZR);//CPFZR
...
CREATEUSER AS 工号，CREATETIME AS 时间 from SQM_TRACKER t order by CREATETIME desc";
        ...
        public ActionResult getTJZLCS()
        {
            string sql = @"select ID, DESCRIPTION from MDM_DIMWP";
            DataTable dt = DataHelper.QueryDataTable(sql);
            return Content(new JsonMessage { Data = dt }.ToString());
        }

        public DataTable FMBJ(string RID, string alonefee)
        {
            string djrid = "";
            string bjrid = "";
            string sql = "";
            ...
            string jxjc = "";
            string fsfeeunit = "";
            string gdzkey = null;// Request.QueryString["gdzkey"];
            string gdzrid = null;// Request.QueryString["gdzrid"];
            bool djqtfs = false;
            bool min = false;
            bool gdznum = true;
            ...
            sql = @"select p.BJSTATAUS,p.BJFS,p.DISCOUNT,p.OTHER_NAME,p.STAGETYPE,p.MRID,p.VRID,p.FEE_CODE,p.FEE_NAME,p.ISLSC,p.CONDITION,p.JXJC,p.JSFCODE,p.JSF,p.JSFJS,p.ISLSC,
                    p.MINSTATUS,to_char(p.BJSTARTDATE,'yyyy/mm/dd') as BJSTARTDATE,to_char(p.BJENDDATE,'yyyy/mm/dd') as BJENDDATE,p.JSFJSCODE,
                    to_char(b.DTFROM, 'yyyy/mm/dd') as DTFROM,to_char(b.DTTO, 'yyyy/mm/dd') as DTTO 
            ...
            sql = @"With DATASET AS(
                           ...
                           where sdf.RID='" + djrid + "') select distinct sfpr.DJFSRID,sfpr.DJFSNAME,sfpr.FSSORT,sfpr.FSDISP from DATASET t1 left join SQM_FEE_PUR_REF sfpr on t1.RID=sfpr.feerid  and sfpr.STATUS='1' where sfpr.DJFSRID is not null order by cast(sfpr.FSSORT as int) asc,sfpr.DJFSNAME asc";
        ...
        private int getFieldKeysupdate(string bjrid, bool min, string where, ref List<string> calccodestrc)
        {
            try
            {
                ...
                string update = @"update SQM_FEE_CALC_REF r left join SQM_BJ_PSF p on r.feecode=p.fee_code
set r.MSRCODE=1000 where p.Rid='{0}' and r.STATUS='1' {1} order by r.SORD asc
";
                string sql = string.Format(update, bjrid, where);
                var dt = DataHelper.ExecSql(sql);
            }
            catch (Exception e)
            {
                return -1;
